\section{CHƯƠNG 1: GIỚI THIỆU ĐỀ TÀI}

\subsection{Giới thiệu đề tài}
Trong quá trình quản lý dự án, việc tổ chức và theo dõi công việc luôn là một bài toán phức tạp đối với các cá nhân hay tổ chức. Khi số lượng công việc gia tăng, các phương pháp quản lý truyền thống hoặc rời rạc thường bộc lộ nhiều hạn chế, dẫn đến những khó khăn trực tiếp trong việc kiểm soát tiến độ, phân chia nguồn lực và duy trì sự phối hợp giữa các thành viên. Tình trạng thất lạc thông tin, thiếu minh bạch về trạng thái thực hiện các hạng mục công việc làm suy giảm đáng kể hiệu suất chung. Xuất phát từ thực trạng này, bài toán đặt ra là cần xây dựng một giải pháp phần mềm hỗ trợ quản lý dự án tập trung, trực quan, giúp người dùng dễ dàng tổ chức các đầu việc, theo dõi tiến độ và nâng cao hiệu quả cộng tác.

\subsection{Tính cấp thiết của đề tài}
Hiện nay, mặc dù đã có nhiều giải pháp phần mềm hỗ trợ quản lý, nhiều cá nhân và tổ chức tại Việt Nam vẫn sử dụng các phương pháp thủ công như ghi chép sổ tay, bảng trắng, hoặc các công cụ kỹ thuật số rời rạc và thiếu tính trực quan như email, bảng tính Excel, hay các ứng dụng nhắn tin nhanh. Các phương thức tiếp cận truyền thống này bộc lộ nhiều hạn chế nghiêm trọng trong môi trường làm việc hiện đại:
\begin{itemize}
    \item \textbf{Thất lạc và phân mảnh thông tin:} Các yêu cầu công việc, phản hồi và tài liệu đính kèm bị phân tán trên nhiều kênh giao tiếp khác nhau, dẫn đến khó khăn trong việc tìm kiếm và dễ bỏ sót nhiệm vụ.
    \item \textbf{Khó theo dõi tiến độ tổng thể:} Người quản lý và các thành viên gặp nhiều khó khăn để có được cái nhìn tổng quan về trạng thái hiện tại của dự án, dẫn đến chậm trễ tiến độ mà không có biện pháp cảnh báo hoặc xử lý kịp thời.
    \item \textbf{Thiếu tính tương tác trực quan:} Các công cụ truyền thống không hỗ trợ cập nhật động trạng thái công việc và luồng di chuyển của các nhiệm vụ theo thời gian thực.
    \item \textbf{Giảm hiệu suất cộng tác nhóm:} Việc phân chia trách nhiệm và giao việc không được định nghĩa rõ ràng, gây chồng chéo công việc và suy giảm hiệu quả làm việc chung.
\end{itemize}
Từ những hạn chế trên, việc nghiên cứu và xây dựng ``Hệ thống quản lý công việc'' là vô cùng cấp thiết. Hệ thống được phát triển nhằm khắc phục triệt để các nhược điểm của phương pháp truyền thống bằng việc cung cấp một giao diện quản lý trực quan theo mô hình Kanban. Giải pháp này giúp chuẩn hóa quy trình giao việc, nâng cao tính minh bạch trong tập thể, tối ưu hóa quy trình làm việc và giảm thiểu tối đa các sai sót thông tin trong suốt quá trình vận hành dự án.

\subsection{Mục tiêu đề tài}
Mục tiêu chính của đề tài là nghiên cứu, thiết kế và phát triển một nền tảng quản lý công việc và thẻ công việc (task/card) hiệu quả, trực quan, dễ tiếp cận và có khả năng tương tác cao. Các mục tiêu chi tiết bao gồm:
\begin{itemize}
    \item \textbf{Xây dựng nền tảng quản lý khoa học:} Cho phép người dùng dễ dàng tổ chức công việc theo các không gian làm việc (workspace) riêng biệt và các bảng công việc (board) theo từng dự án cụ thể.
    \item \textbf{Quản lý thẻ công việc chi tiết:} Cung cấp đầy đủ các thuộc tính quản lý cho một nhiệm vụ như tiêu đề, mô tả chi tiết, gán thành viên thực hiện, thiết lập thời hạn hoàn thành (due date) và xây dựng các danh sách kiểm tra công việc con (checklist) để theo dõi chi tiết.
    \item \textbf{Tích hợp cơ chế tương tác trực quan:} Triển khai thành công tính năng kéo thả (drag and drop) các thẻ công việc giữa các danh sách trạng thái khác nhau, mang lại trải nghiệm mượt mà và trực quan cho người dùng.
    \item \textbf{Đồng bộ dữ liệu và cộng tác hiệu quả:} Đảm bảo các cập nhật về trạng thái công việc, phân công nhân sự và thời hạn được đồng bộ hóa nhanh chóng đến toàn bộ thành viên trong bảng công việc.
\end{itemize}

\subsection{Phạm vi đề tài}
Đề tài tập trung nghiên cứu và triển khai các chức năng cốt lõi phục vụ nhu cầu quản lý công việc hàng ngày của cá nhân và đội ngũ, cụ thể bao gồm:
\begin{itemize}
    \item \textbf{Hệ thống xác thực và quản lý tài khoản:} Triển khai các tính năng đăng ký, đăng nhập, đăng xuất và cập nhật thông tin cá nhân của người dùng, tích hợp giải pháp xác thực bảo mật thông qua dịch vụ điện toán đám mây Amazon Cognito.
    \item \textbf{Quản lý Workspace:} Tạo lập, cập nhật thông tin, xóa và tổ chức các không gian làm việc để quản lý tập trung nhiều dự án liên quan.
    \item \textbf{Quản lý Board:} Tạo mới, cập nhật thông tin, xóa các bảng công việc theo dự án; triển khai tính năng tìm kiếm và thêm thành viên vào bảng công việc.
    \item \textbf{Quản lý danh sách công việc (List):} Tạo mới, đổi tên, thay đổi thứ tự sắp xếp và xóa các danh sách trong mỗi bảng nhằm phân loại trạng thái công việc (ví dụ: Cần làm, Đang làm, Đã hoàn thành).
    \item \textbf{Quản lý thẻ công việc (Card):} Thực hiện các thao tác CRUD (Tạo, Đọc, Cập nhật, Xóa) đối với thẻ công việc; hỗ trợ thêm mô tả chi tiết, gán thành viên chịu trách nhiệm, thiết lập thời hạn hoàn thành và hiển thị trạng thái cảnh báo trực quan khi công việc sắp đến hạn hoặc quá hạn.
    \item \textbf{Quản lý danh sách kiểm tra (Checklist):} Cho phép tạo các checklist trong mỗi card, thêm các hạng mục công việc con (checklist item), đánh dấu hoàn thành/chưa hoàn thành và hiển thị thanh tiến độ (progress bar) trực quan theo tỷ lệ phần trăm hoàn thành.
    \item \textbf{Tương tác kéo thả (Drag and Drop):} Phát triển thuật toán và giao diện hỗ trợ kéo thả thẻ công việc giữa các danh sách hoặc thay đổi vị trí của các thẻ trong cùng một danh sách, đảm bảo tính nhất quán về vị trí hiển thị giữa Frontend và Backend.
\end{itemize}
Các nội dung và tính năng nằm ngoài phạm vi nghiên cứu thực hiện của học phần bao gồm:
\begin{itemize}
    \item Các chức năng phân tích số liệu nâng cao, dự báo tiến độ và tự động xuất các báo cáo thống kê định kỳ dưới dạng biểu đồ phức tạp.
    \item Việc tích hợp sâu hệ thống quản lý công việc với các nền tảng và dịch vụ của bên thứ ba như Slack, Google Calendar, hay các công cụ lưu trữ đám mây bên ngoài.
    \item Tối ưu hóa hạ tầng và cơ chế cân bằng tải để phục vụ cho hệ thống quy mô lớn có hàng triệu người dùng hoạt động đồng thời.
\end{itemize}

\subsection{Đối tượng sử dụng}
Hệ thống quản lý công việc hướng đến việc giải quyết nhu cầu của các nhóm đối tượng người dùng chính sau:
\begin{itemize}
    \item \textbf{Cá nhân:} Người dùng có nhu cầu sắp xếp, lên kế hoạch và quản lý các công việc học tập, nghiên cứu hoặc làm việc cá nhân hàng ngày một cách khoa học, giúp tăng tính kỷ luật và hiệu suất cá nhân.
    \item \textbf{Nhóm làm việc và phòng ban:} Các đội ngũ phát triển dự án, nhóm nghiên cứu hoặc các phòng ban chức năng trong tổ chức cần một không gian cộng tác số thống nhất để phân công công việc, phối hợp thực hiện và cùng theo dõi tiến độ công việc một cách minh bạch, rõ ràng.
    \item \textbf{Quản lý dự án:} Những người chịu trách nhiệm điều phối và giám sát toàn bộ hoạt động của dự án, cần theo dõi tổng quan tiến độ của từng hạng mục công việc, kiểm soát thời hạn hoàn thành (deadlines) và tối ưu hóa việc phân bổ nguồn lực nhân sự.
\end{itemize}
\clearpage

\setcounter{secnumdepth}{4}
\setcounter{tocdepth}{4}
\section{CHƯƠNG 2: CƠ SỞ LÝ THUYẾT}

\subsection{Cách tiếp cận và hướng giải quyết}

\subsubsection{Phương pháp phân tích bài toán}
Bài toán được tiếp cận theo hướng phân tích từ trên xuống (top-down), bắt đầu từ việc nghiên cứu các hệ thống quản lý công việc phổ biến hiện có trên thị trường như Trello và Jira, từ đó xác định mô hình nghiệp vụ, các thực thể dữ liệu cốt lõi và luồng hoạt động của hệ thống. Qua nghiên cứu, mô hình Kanban được lựa chọn làm nền tảng tổ chức công việc do tính trực quan và khả năng phản ánh trạng thái công việc một cách rõ ràng theo các cột trạng thái (To Do, In Progress, Done).

Mô hình dữ liệu nghiệp vụ được xác định xoay quanh sáu thực thể chính theo cấu trúc phân cấp:
\begin{itemize}
    \item \textbf{Workspace}: không gian làm việc tổng thể của người dùng, quản lý tập trung nhiều dự án liên quan.
    \item \textbf{Board}: đại diện cho một dự án hoặc bảng công việc cụ thể bên trong Workspace.
    \item \textbf{List}: các cột trạng thái bên trong Board, thể hiện giai đoạn thực hiện của công việc.
    \item \textbf{Card}: nhiệm vụ cụ thể cần thực hiện, được đặt trong các List tương ứng.
    \item \textbf{Checklist}: danh sách các công việc con nằm bên trong thẻ công việc (Card), hỗ trợ theo dõi chi tiết tiến trình thực hiện.
    \item \textbf{ChecklistItem}: các hạng mục cụ thể bên trong Checklist, có thể được đánh dấu trạng thái hoàn thành.
\end{itemize}

\subsubsection{Hướng thiết kế giải pháp}
Giải pháp được thiết kế theo kiến trúc client-server với sự tách biệt rõ ràng giữa Frontend và Backend thông qua giao tiếp RESTful API. Hướng tiếp cận này giúp hai thành phần có thể phát triển song song, dễ dàng bảo trì và mở rộng độc lập. Toàn bộ logic nghiệp vụ được tập trung xử lý tại Backend, trong khi Frontend chịu trách nhiệm trình bày giao diện và tương tác với người dùng.

Đối với bài toán kéo thả (drag and drop), giải pháp sử dụng thuật toán quản lý vị trí (position) dựa trên trường số nguyên trong cơ sở dữ liệu. Khi người dùng di chuyển một thẻ công việc, hệ thống tính toán và cập nhật lại giá trị \texttt{position} của thẻ đó tương ứng với vị trí đích, đảm bảo tính nhất quán giữa thứ tự hiển thị trên giao diện và dữ liệu lưu trữ trong cơ sở dữ liệu.

\subsubsection{Các quyết định kỹ thuật chính}
Các quyết định kỹ thuật quan trọng được đưa ra trong quá trình thiết kế hệ thống bao gồm:
\begin{itemize}
    \item \textbf{Phân tách Frontend và Backend}: Frontend xây dựng bằng ReactJS, Backend xây dựng bằng Java Spring Boot, giao tiếp qua REST API, tạo ra sự linh hoạt cao trong phát triển và triển khai.
    \item \textbf{Ủy quyền xác thực cho dịch vụ bên thứ ba}: Sử dụng Amazon Cognito để quản lý toàn bộ vòng đời người dùng (đăng ký, xác thực email, đăng nhập), giúp Backend tập trung vào logic nghiệp vụ thay vì tự xử lý xác thực.
    \item \textbf{Lưu trữ token bằng HttpOnly Cookie}: Access token và refresh token được lưu trong HttpOnly cookie thay vì localStorage, ngăn chặn các cuộc tấn công XSS (Cross-Site Scripting).
    \item \textbf{Cơ sở dữ liệu quan hệ PostgreSQL}: Phù hợp với mô hình dữ liệu có cấu trúc quan hệ rõ ràng và hỗ trợ tốt cho JPA/Hibernate trong Spring Boot.
    \item \textbf{Triển khai trên nền tảng Cloud}: Ứng dụng được triển khai hoàn toàn lên môi trường điện toán đám mây thông qua hệ sinh thái các dịch vụ của AWS (bao gồm EC2, RDS, S3, CloudFront và ALB), giúp hệ thống tận dụng tối đa khả năng mở rộng linh hoạt, tối ưu chi phí hạ tầng và đảm bảo tính sẵn sàng cao.
\end{itemize}

\subsection{Các nền tảng và công nghệ sử dụng}

\subsubsection{Các nền tảng liên quan đến ứng dụng}

\paragraph{ReactJS}

\begin{itemize}
    \item \textbf{Giới thiệu:} ReactJS là thư viện JavaScript mã nguồn mở được phát triển bởi Meta (Facebook), chuyên dùng để xây dựng giao diện người dùng (UI) tương tác và có hiệu suất cao.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{./images/chap2/React.jpg}
    \caption{Logo ReactJS}
    \label{fig:reactjs_logo}
\end{figure}
    \item \textbf{Chức năng:} ReactJS hoạt động theo cơ chế component-based, cho phép chia nhỏ giao diện thành các thành phần độc lập, có thể tái sử dụng. Virtual DOM giúp cập nhật giao diện một cách hiệu quả bằng cách chỉ render lại những phần thực sự thay đổi, thay vì cập nhật toàn bộ cây DOM.
    \item \textbf{Vai trò trong hệ thống:} ReactJS được sử dụng để xây dựng toàn bộ tầng giao diện (View layer) của hệ thống quản lý công việc, bao gồm các màn hình xác thực (đăng ký, đăng nhập), màn hình Dashboard hiển thị danh sách Workspace và Board, cũng như màn hình Board chi tiết với các chức năng quản lý List, Card và Drag and Drop.
    \item \textbf{Lý do lựa chọn:} Đối với các dạng ứng dụng web trang đơn (Single Page Application - SPA), ReactJS là một thư viện phổ biến, được sử dụng rất nhiều bởi các lập trình viên nhờ tính linh hoạt và khả năng triển khai dễ dàng, nhanh chóng.
\end{itemize}

\paragraph{Java Spring Boot}

\begin{itemize}
    \item \textbf{Giới thiệu:} Spring Boot là framework phát triển ứng dụng Java được xây dựng trên nền tảng Spring Framework, cung cấp cơ chế tự động cấu hình (auto-configuration) giúp giảm thiểu thời gian thiết lập dự án và tập trung vào triển khai logic nghiệp vụ.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{./images/chap2/SpringBoot.jpg}
    \caption{Logo Java Spring Boot}
    \label{fig:springboot_logo}
\end{figure}
    \item \textbf{Chức năng:} Spring Boot hỗ trợ xây dựng các ứng dụng RESTful API thông qua module Spring Web, quản lý kết nối cơ sở dữ liệu thông qua Spring Data JPA và Hibernate, đồng thời cung cấp cơ chế dependency injection giúp tổ chức code theo hướng lập trình hướng đối tượng rõ ràng.
    \item \textbf{Vai trò trong hệ thống:} Spring Boot đóng vai trò là tầng Backend của hệ thống, xử lý toàn bộ logic nghiệp vụ, xây dựng và cung cấp các RESTful API cho Frontend tiêu thụ. Cấu trúc project được tổ chức theo mô hình phân lớp bao gồm: \texttt{controller}, \texttt{service}, \texttt{repository} và \texttt{config}.
    \item \textbf{Lý do lựa chọn:} Spring Boot là một framework hỗ trợ công việc viết code cho các lập trình viên một cách mạnh mẽ và tối ưu. Kết hợp cùng với Java - một ngôn ngữ lập trình chặt chẽ, có tính ổn định cao, việc sử dụng Spring Boot giúp giảm thiểu tối đa các lỗi hệ thống có thể xảy ra trong quá trình vận hành.
\end{itemize}

\paragraph{PostgreSQL}

\begin{itemize}
    \item \textbf{Giới thiệu:} PostgreSQL là hệ quản trị cơ sở dữ liệu quan hệ mã nguồn mở mạnh mẽ, hỗ trợ đầy đủ các tính năng của SQL chuẩn cùng nhiều tính năng nâng cao như kiểu dữ liệu JSON, UUID, các ràng buộc phức tạp và transaction ACID.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{./images/chap2/PostgreSQL.png}
    \caption{Logo PostgreSQL}
    \label{fig:postgresql_logo}
\end{figure}
    \item \textbf{Chức năng:} PostgreSQL đảm bảo tính toàn vẹn dữ liệu thông qua các ràng buộc FK (foreign key), hỗ trợ các truy vấn phức tạp với nhiều bảng quan hệ, đồng thời cung cấp hiệu suất đọc/ghi ổn định phù hợp với các ứng dụng web thông thường.
    \item \textbf{Vai trò trong hệ thống:} PostgreSQL được sử dụng làm hệ quản trị cơ sở dữ liệu chính của hệ thống, lưu trữ toàn bộ dữ liệu nghiệp vụ bao gồm thông tin người dùng, Workspace, Board, List, Card, Checklist và các bảng quan hệ trung gian. Kết nối với Backend thông qua Spring Data JPA và trình điều khiển PostgreSQL JDBC.
\end{itemize}

\paragraph{Mô hình MVC (Model -- View -- Controller)}
\begin{itemize}
    \item \textbf{Giới thiệu:} MVC là mẫu kiến trúc phần mềm (architectural pattern) phân tách ứng dụng thành ba thành phần độc lập: \textbf{Model} chịu trách nhiệm quản lý dữ liệu và logic nghiệp vụ; \textbf{View} đảm nhận việc trình bày dữ liệu ra giao diện người dùng; \textbf{Controller} đóng vai trò trung gian, tiếp nhận yêu cầu từ người dùng, điều phối Model xử lý và trả kết quả về View.
    \item \textbf{Chức năng:} Mô hình MVC thúc đẩy nguyên tắc tách biệt mối quan tâm (Separation of Concerns), giúp từng thành phần có thể phát triển, kiểm thử và bảo trì một cách độc lập. Sự phân tách này làm giảm độ phức tạp của mã nguồn, tăng khả năng tái sử dụng và hỗ trợ làm việc song song giữa các thành viên trong nhóm phát triển.
    \item \textbf{Vai trò trong hệ thống:} Mô hình MVC được áp dụng ở tầng Backend (Spring Boot). Trong đó, \textbf{Controller} là tầng tiếp nhận các HTTP request từ Frontend và điều phối xử lý; \textbf{Model} bao gồm các Entity JPA ánh xạ trực tiếp đến bảng dữ liệu PostgreSQL và các lớp DTO truyền dữ liệu giữa các tầng; \textbf{View} trong ngữ cảnh RESTful API được thay thế bởi phản hồi JSON chuẩn hóa trả về cho Frontend. Cấu trúc project được tổ chức thành các package \texttt{controller}, \texttt{service}, \texttt{repository} và \texttt{model} nhất quán theo nguyên tắc này.
\end{itemize}

\subsubsection{Các nền tảng liên quan đến dịch vụ đám mây}

\paragraph{Amazon Cognito}
\begin{itemize}
    \item \textbf{Giới thiệu:} Amazon Cognito là dịch vụ quản lý danh tính và xác thực người dùng do Amazon Web Services (AWS) cung cấp, hỗ trợ các luồng xác thực theo chuẩn OAuth 2.0 và OpenID Connect.
    \item \textbf{Chức năng:} Amazon Cognito cung cấp hai thành phần chính: User Pool để quản lý người dùng và xác thực danh tính, và Identity Pool để cấp quyền truy cập tài nguyên AWS. Dịch vụ này tự động xử lý việc gửi email xác nhận, quản lý mật khẩu, làm mới token và bảo vệ tài khoản theo các chính sách bảo mật chuẩn.
    \item \textbf{Vai trò trong hệ thống:} Amazon Cognito được tích hợp trực tiếp với tầng Backend thông qua AWS SDK for Java để xử lý toàn bộ luồng xác thực người dùng. Theo kiến trúc này, Client không gọi trực tiếp đến Cognito mà tương tác qua Backend; Backend sẽ giao tiếp với Cognito để đăng nhập, đăng ký và nhận JWT (access token, refresh token), sau đó trả về cho Client.
\end{itemize}

\paragraph{Amazon RDS}
\begin{itemize}
    \item \textbf{Giới thiệu:} Amazon RDS (Relational Database Service) là dịch vụ cơ sở dữ liệu quan hệ được quản lý hoàn toàn bởi AWS, hỗ trợ nhiều hệ quản trị cơ sở dữ liệu phổ biến trong đó có PostgreSQL.
    \item \textbf{Chức năng:} Amazon RDS tự động hóa các tác vụ quản trị cơ sở dữ liệu như sao lưu định kỳ, vá lỗi phần mềm, phát hiện và khôi phục lỗi tự động (automatic failure detection and recovery) và mở rộng tài nguyên. Dịch vụ cũng cung cấp tính năng High Availability thông qua triển khai Multi-AZ, đảm bảo cơ sở dữ liệu luôn sẵn sàng phục vụ.
    \item \textbf{Vai trò trong hệ thống:} Amazon RDS được dự kiến sử dụng để hosting cơ sở dữ liệu PostgreSQL trên môi trường đám mây, thay thế cho cơ sở dữ liệu cục bộ trong quá trình phát triển, nhằm chuẩn bị cho việc triển khai hệ thống lên môi trường Staging trong giai đoạn tiếp theo.
\end{itemize}

\paragraph{Amazon EC2 (Elastic Compute Cloud)}
\begin{itemize}
    \item \textbf{Giới thiệu:} Amazon EC2 là một dịch vụ web cốt lõi của AWS, cung cấp khả năng điện toán (compute capacity) an toàn và có thể thay đổi kích thước linh hoạt trên đám mây.
    \item \textbf{Chức năng:} EC2 cho phép khởi tạo các máy chủ ảo (instances) để chạy các ứng dụng. Người dùng có toàn quyền kiểm soát cấu hình phần cứng ảo (CPU, RAM), hệ điều hành và cài đặt các phần mềm cần thiết trên đó.
    \item \textbf{Vai trò trong hệ thống:} EC2 được sử dụng làm máy chủ (đặt trong mạng nội bộ - Private Subnet) để triển khai cả ứng dụng Backend (chạy Spring Boot) và Frontend (chạy ReactJS qua Nginx và Docker). Việc đặt EC2 trong mạng private giúp cách ly hoàn toàn các máy chủ khỏi Internet, tăng cường tối đa tính bảo mật.
\end{itemize}

\paragraph{Amazon ALB (Application Load Balancer)}
\begin{itemize}
    \item \textbf{Giới thiệu:} Amazon ALB là một dịch vụ cân bằng tải ở tầng ứng dụng (Layer 7) của AWS, chuyên phân phối lưu lượng truy cập HTTP và HTTPS một cách thông minh.
    \item \textbf{Chức năng:} ALB có khả năng phân phối lưu lượng truy cập mạng từ bên ngoài đến nhiều máy chủ đích (như các EC2 instances). Nó cung cấp các tính năng như định tuyến linh hoạt, hỗ trợ bảo mật SSL/TLS và tự động kiểm tra sức khỏe (health check) của các máy chủ đích để loại bỏ các máy chủ đang gặp lỗi.
    \item \textbf{Vai trò trong hệ thống:} ALB đóng vai trò là điểm truy cập (entry point) duy nhất cho toàn bộ hệ thống, tiếp nhận các request từ người dùng và phân phối tải đều đến các máy chủ EC2. Điều này giúp nâng cao tính sẵn sàng, cải thiện hiệu suất và hỗ trợ mở rộng hệ thống một cách dễ dàng khi lưu lượng truy cập tăng cao.
\end{itemize}

\paragraph{Amazon S3 (Simple Storage Service)}
\begin{itemize}
    \item \textbf{Giới thiệu:} Amazon S3 là dịch vụ lưu trữ đối tượng (object storage) do AWS cung cấp, mang lại khả năng lưu trữ và truy xuất lượng dữ liệu bất kỳ từ mọi nơi trên Internet với độ bền và tính bảo mật cao.
    \item \textbf{Chức năng:} S3 cho phép lưu trữ không giới hạn các loại tệp tin tĩnh (static files) và dữ liệu phi cấu trúc. Nó cũng hỗ trợ tính năng lưu trữ website tĩnh (static website hosting) để phục vụ trực tiếp các tệp HTML, CSS, JavaScript cho các ứng dụng web.
    \item \textbf{Vai trò trong hệ thống:} Trong dự án này, do Frontend đã được đưa lên máy chủ EC2, Amazon S3 được sử dụng chuyên biệt làm nơi lưu trữ an toàn cho các tài nguyên đa phương tiện do người dùng tải lên, ví dụ như hình ảnh đại diện (avatar) hoặc các tệp đính kèm trong thẻ công việc (Card).
\end{itemize}

\paragraph{Amazon CloudFront}
\begin{itemize}
    \item \textbf{Giới thiệu:} Amazon CloudFront là dịch vụ mạng phân phối nội dung (Content Delivery Network - CDN) tốc độ cao của AWS, giúp truyền tải dữ liệu và ứng dụng tới người dùng trên toàn cầu với độ trễ cực thấp.
    \item \textbf{Chức năng:} CloudFront lưu trữ bộ nhớ đệm (caching) các nội dung tĩnh và động tại các vị trí biên (edge locations) trên khắp thế giới, giúp rút ngắn khoảng cách vật lý giữa người dùng và dữ liệu, từ đó cải thiện đáng kể tốc độ tải trang web.
    \item \textbf{Vai trò trong hệ thống:} CloudFront được cấu hình để chỉ tương tác trực tiếp với Client và Amazon S3. Nó đóng vai trò trung gian phân phối các tệp tin đa phương tiện (từ S3) đến tay người dùng một cách tối ưu nhất thông qua bộ nhớ đệm (caching), đồng thời cung cấp giao thức HTTPS bảo mật và hạn chế truy cập công khai vào S3.
\end{itemize}

\paragraph{JWT (JSON Web Token)}
\begin{itemize}
    \item \textbf{Giới thiệu:} JWT là tiêu chuẩn mở (RFC 7519) định nghĩa cách truyền tải thông tin an toàn giữa các bên dưới dạng một đối tượng JSON được ký số (digitally signed).
    \item \textbf{Chức năng:} Một JWT bao gồm ba phần được mã hóa Base64 và ngăn cách bởi dấu chấm: Header (loại thuật toán ký), Payload (thông tin người dùng và thời hạn token) và Signature (chữ ký xác thực). Token có thể được xác minh mà không cần truy vấn cơ sở dữ liệu, giúp giảm tải cho hệ thống.
    \item \textbf{Vai trò trong hệ thống:} JWT được Amazon Cognito phát hành và trả về cho Backend sau khi người dùng đăng nhập thành công. Backend sau đó trả JWT về cho Client để lưu trữ trong HttpOnly cookie. Backend sử dụng token này để xác thực chữ ký trên mỗi request gọi API tiếp theo nhằm đảm bảo tính an toàn.
\end{itemize}

\subsubsection{Các nền tảng khác}

\paragraph{Docker}
\begin{itemize}
    \item \textbf{Giới thiệu:} Docker là nền tảng container hóa mã nguồn mở cho phép đóng gói ứng dụng cùng toàn bộ các phụ thuộc (thư viện, cấu hình, runtime) vào một đơn vị độc lập gọi là container. Container Docker có thể chạy nhất quán trên mọi môi trường từ máy phát triển đến máy chủ sản xuất.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{./images/chap2/Docker.png}
    \caption{Logo Docker}
    \label{fig:docker_logo}
\end{figure}
    \item \textbf{Chức năng:} Docker cung cấp cơ chế cô lập tiến trình thông qua namespace và cgroup của nhân Linux, giúp nhiều container chạy song song trên cùng một máy chủ mà không xung đột tài nguyên. Dockerfile cho phép định nghĩa môi trường build theo dạng khai báo, đảm bảo tính tái tạo (reproducibility) của quá trình build.
    \item \textbf{Vai trò trong hệ thống:} Ứng dụng Frontend (ReactJS) được đóng gói thành Docker image và triển khai trên máy chủ EC2. Docker đảm bảo môi trường chạy ứng dụng nhất quán giữa môi trường phát triển cục bộ và môi trường triển khai trên AWS, đồng thời đơn giản hóa quy trình cập nhật phiên bản ứng dụng.
\end{itemize}

\paragraph{Nginx}
\begin{itemize}
    \item \textbf{Giới thiệu:} Nginx là web server hiệu năng cao và reverse proxy mã nguồn mở, nổi tiếng với khả năng xử lý đồng thời lượng lớn kết nối với mức tiêu thụ tài nguyên thấp nhờ kiến trúc hướng sự kiện bất đồng bộ (event-driven, asynchronous).
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{./images/chap2/Nginx.jpg}
    \caption{Logo Nginx}
    \label{fig:nginx_logo}
\end{figure}
    \item \textbf{Chức năng:} Nginx đảm nhiệm các vai trò phục vụ tệp tĩnh (static file serving), reverse proxy chuyển tiếp request đến ứng dụng phía sau, cấu hình HTTPS và gzip compression để giảm kích thước phản hồi. Ngoài ra, Nginx hỗ trợ cấu hình định tuyến linh hoạt thông qua file cấu hình theo cú pháp khai báo.
    \item \textbf{Vai trò trong hệ thống:} Nginx được cài đặt bên trong Docker container trên máy chủ EC2 Frontend, đóng vai trò là web server phục vụ các tệp tĩnh của ứng dụng ReactJS (HTML, CSS, JavaScript). Ngoài ra, Nginx xử lý định tuyến phía client (client-side routing) bằng cách chuyển hướng mọi request về file \texttt{index.html}, đảm bảo ứng dụng Single Page Application hoạt động đúng khi người dùng truy cập trực tiếp vào các đường dẫn con.
\end{itemize}

\clearpage

\section{CHƯƠNG 3: PHÂN TÍCH VÀ THIẾT KẾ HỆ THỐNG}

\subsection{Phân tích yêu cầu}

\subsubsection{Yêu cầu chức năng}

Hệ thống quản lý công việc Nello được xây dựng để đáp ứng các nhóm chức năng nghiệp vụ sau:

\begin{table}[H]
\centering
\begin{tabular}{|p{2.5cm}|p{5cm}|p{7.5cm}|}
\hline
\textbf{Use Case ID} & \textbf{Tên Use Case} & \textbf{Mô tả ngắn} \\
\hline
UC-001 & Đăng ký tài khoản & Người dùng tạo tài khoản mới bằng email và mật khẩu thông qua Amazon Cognito. \\
\hline
UC-002 & Xác nhận tài khoản & Người dùng nhập mã OTP gửi qua email để kích hoạt tài khoản. \\
\hline
UC-003 & Đăng nhập & Người dùng xác thực và nhận JWT lưu trong HttpOnly Cookie. \\
\hline
UC-004 & Cập nhật thông tin cá nhân & Người dùng chỉnh sửa họ tên, ảnh đại diện, số điện thoại và địa chỉ. \\
\hline
UC-005 & Quản lý Workspace & Người dùng tạo, xem, cập nhật và xóa không gian làm việc. \\
\hline
UC-006 & Mời thành viên vào Workspace & ADMIN thêm người dùng khác vào Workspace với vai trò MEMBER. \\
\hline
UC-007 & Quản lý Board & ADMIN tạo, xem, cập nhật và xóa bảng công việc trong Workspace. \\
\hline
UC-008 & Thêm thành viên vào Board & ADMIN Board thêm thành viên thuộc Workspace vào Board với vai trò xác định. \\
\hline
UC-009 & Quản lý List & ADMIN/MEMBER tạo, đổi tên, xóa và sắp xếp lại các cột trạng thái trong Board. \\
\hline
UC-010 & Quản lý Card & ADMIN/MEMBER tạo, cập nhật, xóa thẻ công việc với đầy đủ thông tin chi tiết. \\
\hline
UC-011 & Kéo thả Card và List & ADMIN/MEMBER di chuyển Card giữa các List hoặc thay đổi thứ tự bằng thao tác kéo thả. \\
\hline
UC-012 & Quản lý Checklist & ADMIN/MEMBER tạo và quản lý danh sách công việc con trong Card. \\
\hline
UC-013 & Gán thành viên vào Card & ADMIN/MEMBER gán người chịu trách nhiệm thực hiện Card. \\
\hline
\end{tabular}
\caption{Danh sách Use Case của hệ thống}
\label{tab:use_case_list}
\end{table}

\subsubsection{Yêu cầu phi chức năng}

\begin{itemize}
    \item \textbf{Bảo mật:} Toàn bộ phiên làm việc được bảo vệ bằng JWT lưu trong HttpOnly Cookie, ngăn chặn tấn công XSS. Xác thực và quản lý mật khẩu được ủy thác cho Amazon Cognito, đảm bảo tuân thủ các tiêu chuẩn bảo mật hiện đại. Phân quyền theo vai trò (ADMIN, MEMBER, VIEWER) được kiểm soát tại tầng Backend trước mỗi thao tác.
    \item \textbf{Hiệu năng:} Giao diện người dùng phản hồi nhanh nhờ cơ chế cập nhật cục bộ trên Frontend trước khi đồng bộ với Backend. Trường \texttt{position} kiểu TEXT cho phép sắp xếp lại thứ tự mà không cần cập nhật toàn bộ bảng.
    \item \textbf{Khả năng mở rộng:} Kiến trúc phân tách Frontend và Backend cho phép mở rộng độc lập từng thành phần. Hạ tầng triển khai trên AWS (EC2, RDS, ALB) hỗ trợ tăng/giảm tài nguyên linh hoạt theo nhu cầu.
    \item \textbf{Tính sẵn sàng:} Ứng dụng được triển khai trên AWS với ALB đứng trước các máy chủ EC2, đảm bảo khả năng tiếp nhận và phân phối tải ổn định.
    \item \textbf{Khả năng bảo trì:} Backend được tổ chức theo mô hình phân lớp Controller -- Service -- Repository, kết hợp DTO và exception handler chuẩn hóa. Schema cơ sở dữ liệu được quản lý bằng Flyway, đảm bảo tính nhất quán giữa các môi trường.
    \item \textbf{Tính dễ sử dụng:} Giao diện Kanban trực quan, hỗ trợ thao tác kéo thả tự nhiên, phù hợp với người dùng không có kiến thức kỹ thuật chuyên sâu.
\end{itemize}

\subsection{Phân rã chức năng}

Dưới đây trình bày chi tiết các Use Case chính của hệ thống theo định dạng đặc tả.

\begin{table}[H]
\centering
\begin{tabular}{|p{4cm}|p{11cm}|}
\hline
\textbf{Thuộc tính} & \textbf{Nội dung} \\
\hline
Use Case ID & UC-001 \\
\hline
Tên Use Case & Đăng ký tài khoản \\
\hline
Actor & Người dùng chưa có tài khoản \\
\hline
Mục tiêu & Tạo tài khoản mới để sử dụng hệ thống \\
\hline
Tiền điều kiện & Người dùng chưa đăng ký tài khoản \\
\hline
Hậu điều kiện & Tài khoản được tạo trên Amazon Cognito; OTP xác nhận được gửi qua email \\
\hline
Luồng chính & 1. Nhập họ tên, địa chỉ email, mật khẩu \newline 2. Hệ thống gọi Cognito \texttt{signUp()} \newline 3. Cognito tạo tài khoản và gửi mã OTP qua email \newline 4. Chuyển sang màn hình xác nhận OTP \\
\hline
Luồng thay thế & Email đã tồn tại: Hiển thị thông báo lỗi, yêu cầu nhập email khác \newline Mật khẩu không đủ độ mạnh: Hiển thị thông báo yêu cầu \newline Lỗi không xác định: Trả về HTTP 500 \\
\hline
\end{tabular}
\caption{Đặc tả Use Case UC-001: Đăng ký tài khoản}
\label{tab:uc001}
\end{table}

\begin{table}[H]
\centering
\begin{tabular}{|p{4cm}|p{11cm}|}
\hline
\textbf{Thuộc tính} & \textbf{Nội dung} \\
\hline
Use Case ID & UC-002 \\
\hline
Tên Use Case & Xác nhận tài khoản \\
\hline
Actor & Người dùng vừa đăng ký \\
\hline
Mục tiêu & Kích hoạt tài khoản để có thể đăng nhập \\
\hline
Tiền điều kiện & Tài khoản đã được tạo trên Cognito, chưa được xác nhận; OTP đã gửi qua email \\
\hline
Hậu điều kiện & Tài khoản được kích hoạt, người dùng có thể đăng nhập \\
\hline
Luồng chính & 1. Nhập mã OTP nhận qua email \newline 2. Hệ thống gọi Cognito \texttt{confirmSignUp()} \newline 3. Cognito xác nhận mã OTP hợp lệ \newline 4. Tài khoản được kích hoạt, điều hướng về trang đăng nhập \\
\hline
Luồng thay thế & OTP sai hoặc hết hạn: Hiển thị thông báo lỗi \newline Người dùng yêu cầu gửi lại OTP: Gọi endpoint \texttt{/auth/resend-code} \newline Lỗi không xác định: Trả về HTTP 500 \\
\hline
\end{tabular}
\caption{Đặc tả Use Case UC-002: Xác nhận tài khoản}
\label{tab:uc002}
\end{table}

\begin{table}[H]
\centering
\begin{tabular}{|p{4cm}|p{11cm}|}
\hline
\textbf{Thuộc tính} & \textbf{Nội dung} \\
\hline
Use Case ID & UC-003 \\
\hline
Tên Use Case & Đăng nhập \\
\hline
Actor & Người dùng đã có tài khoản \\
\hline
Mục tiêu & Xác thực danh tính và truy cập hệ thống \\
\hline
Tiền điều kiện & Tài khoản đã được tạo và xác nhận \\
\hline
Hậu điều kiện & JWT (access token, refresh token) lưu trong HttpOnly Cookie; người dùng được điều hướng về Dashboard \\
\hline
Luồng chính & 1. Nhập địa chỉ email và mật khẩu \newline 2. Hệ thống gọi Cognito \texttt{initiateAuth()} với phương thức \texttt{USER\_PASSWORD\_AUTH} \newline 3. Cognito trả về access token và refresh token \newline 4. Backend lưu token vào hai HttpOnly Cookie \newline 5. Điều hướng về trang Dashboard \\
\hline
Luồng thay thế & Sai email hoặc mật khẩu: Hiển thị thông báo lỗi xác thực \newline Access token hết hạn trong phiên làm việc: Tự động gọi \texttt{/auth/refresh-token} để gia hạn \newline Lỗi không xác định: Trả về HTTP 500 \\
\hline
\end{tabular}
\caption{Đặc tả Use Case UC-003: Đăng nhập}
\label{tab:uc003}
\end{table}

\begin{table}[H]
\centering
\begin{tabular}{|p{4cm}|p{11cm}|}
\hline
\textbf{Thuộc tính} & \textbf{Nội dung} \\
\hline
Use Case ID & UC-004 \\
\hline
Tên Use Case & Cập nhật thông tin cá nhân \\
\hline
Actor & Người dùng đã đăng nhập \\
\hline
Mục tiêu & Chỉnh sửa thông tin hồ sơ cá nhân \\
\hline
Tiền điều kiện & Người dùng đã đăng nhập, JWT hợp lệ \\
\hline
Hậu điều kiện & Thông tin họ tên, ảnh đại diện, số điện thoại, địa chỉ, bio được cập nhật trong cơ sở dữ liệu \\
\hline
Luồng chính & 1. Truy cập trang hồ sơ cá nhân \newline 2. Hệ thống tải thông tin hiện tại qua \texttt{GET /api/v1/users/me} \newline 3. Chỉnh sửa họ tên, bio, số điện thoại, địa chỉ hoặc URL ảnh đại diện \newline 4. Gửi yêu cầu cập nhật qua \texttt{PUT /api/v1/users} kèm \texttt{UpdateProfileRequest} \newline 5. Hệ thống cập nhật bản ghi trong bảng \texttt{users} và trả về thông tin đã cập nhật \\
\hline
Luồng thay thế & Người dùng không tồn tại: Trả về HTTP 404 \newline Token không hợp lệ: Trả về HTTP 401 \newline Lỗi không xác định: Trả về HTTP 500 \\
\hline
\end{tabular}
\caption{Đặc tả Use Case UC-004: Cập nhật thông tin cá nhân}
\label{tab:uc004}
\end{table}

\begin{table}[H]
\centering
\begin{tabular}{|p{4cm}|p{11cm}|}
\hline
\textbf{Thuộc tính} & \textbf{Nội dung} \\
\hline
Use Case ID & UC-005 \\
\hline
Tên Use Case & Quản lý Workspace \\
\hline
Actor & Người dùng đã đăng nhập \\
\hline
Mục tiêu & Tạo, xem, cập nhật và xóa không gian làm việc để tổ chức các dự án liên quan \\
\hline
Tiền điều kiện & Người dùng đã đăng nhập, JWT hợp lệ \\
\hline
Hậu điều kiện & Workspace được tạo/cập nhật/xóa; trạng thái danh sách Workspace trên Dashboard được đồng bộ \\
\hline
Luồng chính (Tạo) & 1. Nhập tên Workspace, mô tả và chọn loại (category) \newline 2. Hệ thống tạo bản ghi trong bảng \texttt{workspaces} \newline 3. Tự động thêm người tạo vào \texttt{workspace\_members} với vai trò ADMIN \newline 4. Hiển thị Workspace mới trong sidebar \\
\hline
Luồng chính (Xem) & 1. Truy cập Dashboard, hệ thống tải danh sách Workspace qua \texttt{GET /api/v1/workspaces} \newline 2. Truy cập trang chi tiết, tải thông tin qua \texttt{GET /api/v1/workspaces/:id} \\
\hline
Luồng chính (Cập nhật) & 1. Chỉnh sửa tên hoặc mô tả Workspace \newline 2. Hệ thống cập nhật bản ghi và đồng bộ giao diện \\
\hline
Luồng chính (Xóa) & 1. Chọn xóa Workspace sau khi xác nhận \newline 2. Hệ thống xóa Workspace và toàn bộ Board, List, Card bên trong (cascade delete) \\
\hline
Luồng thay thế & Tên Workspace để trống: Trả về HTTP 400 \newline Không có quyền ADMIN: Trả về HTTP 403 \newline Lỗi không xác định: Trả về HTTP 500 \\
\hline
\end{tabular}
\caption{Đặc tả Use Case UC-005: Quản lý Workspace}
\label{tab:uc005}
\end{table}

\begin{table}[H]
\centering
\begin{tabular}{|p{4cm}|p{11cm}|}
\hline
\textbf{Thuộc tính} & \textbf{Nội dung} \\
\hline
Use Case ID & UC-006 \\
\hline
Tên Use Case & Mời thành viên vào Workspace \\
\hline
Actor & ADMIN Workspace \\
\hline
Mục tiêu & Thêm người dùng khác vào Workspace để cộng tác \\
\hline
Tiền điều kiện & Người dùng có vai trò ADMIN trong Workspace \\
\hline
Hậu điều kiện & Người dùng được mời được thêm vào \texttt{workspace\_members} với vai trò MEMBER \\
\hline
Luồng chính & 1. Tìm kiếm người dùng theo email hoặc họ tên qua \texttt{GET /api/v1/users?search=...} \newline 2. Chọn người dùng từ kết quả tìm kiếm \newline 3. Gửi yêu cầu mời qua \texttt{POST /api/v1/workspaces-members/:workspaceId} \newline 4. Hệ thống kiểm tra người dùng chưa là thành viên \newline 5. Tạo bản ghi trong \texttt{workspace\_members} với vai trò MEMBER \newline 6. Cập nhật danh sách thành viên trên giao diện \\
\hline
Luồng thay thế & Người dùng đã là thành viên: Trả về thông báo lỗi \newline Không tìm thấy người dùng: Hiển thị danh sách kết quả rỗng \newline Lỗi không xác định: Trả về HTTP 500 \\
\hline
\end{tabular}
\caption{Đặc tả Use Case UC-006: Mời thành viên vào Workspace}
\label{tab:uc006}
\end{table}

\begin{table}[H]
\centering
\begin{tabular}{|p{4cm}|p{11cm}|}
\hline
\textbf{Thuộc tính} & \textbf{Nội dung} \\
\hline
Use Case ID & UC-007 \\
\hline
Tên Use Case & Quản lý Board \\
\hline
Actor & ADMIN Workspace \\
\hline
Mục tiêu & Tạo, xem, cập nhật và xóa bảng công việc trong Workspace \\
\hline
Tiền điều kiện & Workspace tồn tại; người dùng có vai trò ADMIN trong Workspace \\
\hline
Hậu điều kiện & Board được tạo/cập nhật/xóa; danh sách Board trong Workspace được đồng bộ \\
\hline
Luồng chính (Tạo) & 1. Nhập tên Board và chọn ảnh nền từ 8 ảnh có sẵn \newline 2. Hệ thống kiểm tra quyền ADMIN và tạo bản ghi trong bảng \texttt{boards} \newline 3. Tự động thêm người tạo vào \texttt{board\_members} với vai trò ADMIN \\
\hline
Luồng chính (Xem) & 1. Truy cập trang Board, hệ thống tải toàn bộ List và Card qua \texttt{GET /api/v1/boards/:id} \newline 2. Dữ liệu được sắp xếp theo \texttt{position} và hiển thị theo mô hình Kanban \\
\hline
Luồng chính (Cập nhật) & 1. Chỉnh sửa tên Board \newline 2. Hệ thống cập nhật bản ghi qua \texttt{PUT /api/v1/boards/:id} \\
\hline
Luồng chính (Xóa) & 1. Chọn xóa Board sau khi xác nhận \newline 2. Hệ thống xóa Board và toàn bộ List, Card bên trong (cascade delete) qua \texttt{DELETE /api/v1/boards/:id} \\
\hline
Luồng thay thế & Workspace không tồn tại: Trả về HTTP 404 \newline Người dùng không có quyền ADMIN: Trả về HTTP 403 \newline Lỗi không xác định: Trả về HTTP 500 \\
\hline
\end{tabular}
\caption{Đặc tả Use Case UC-007: Quản lý Board}
\label{tab:uc007}
\end{table}

\begin{table}[H]
\centering
\begin{tabular}{|p{4cm}|p{11cm}|}
\hline
\textbf{Thuộc tính} & \textbf{Nội dung} \\
\hline
Use Case ID & UC-008 \\
\hline
Tên Use Case & Quản lý List \\
\hline
Actor & ADMIN hoặc MEMBER Board \\
\hline
Mục tiêu & Tạo, cập nhật và xóa các cột trạng thái trong Board \\
\hline
Tiền điều kiện & Board tồn tại; người dùng có vai trò ADMIN hoặc MEMBER trong Board \\
\hline
Hậu điều kiện & List được tạo mới, đổi tên, xóa hoặc thứ tự hiển thị được cập nhật \\
\hline
Luồng chính (Tạo) & 1. Nhập tên List \newline 2. Hệ thống tạo List với giá trị \texttt{position} ở cuối danh sách \newline 3. List hiển thị ở cuối Board \\
\hline
Luồng chính (Đổi tên) & 1. Nhấp vào tiêu đề List, nhập tên mới \newline 2. Hệ thống cập nhật trường \texttt{name} trong bảng \texttt{lists} \\
\hline
Luồng chính (Xóa) & 1. Chọn xóa List \newline 2. Hệ thống xóa List và toàn bộ Card bên trong (cascade delete) \\
\hline
Luồng thay thế & Người dùng không có quyền ADMIN/MEMBER: Trả về HTTP 403 \newline Lỗi không xác định: Trả về HTTP 500 \\
\hline
\end{tabular}
\caption{Đặc tả Use Case UC-008: Quản lý List}
\label{tab:uc008}
\end{table}

\begin{table}[H]
\centering
\begin{tabular}{|p{4cm}|p{11cm}|}
\hline
\textbf{Thuộc tính} & \textbf{Nội dung} \\
\hline
Use Case ID & UC-009 \\
\hline
Tên Use Case & Quản lý Card \\
\hline
Actor & ADMIN hoặc MEMBER Board \\
\hline
Mục tiêu & Tạo, cập nhật và xóa thẻ công việc trong List \\
\hline
Tiền điều kiện & List tồn tại; người dùng có vai trò ADMIN hoặc MEMBER trong Board \\
\hline
Hậu điều kiện & Card được tạo mới hoặc thông tin Card được cập nhật \\
\hline
Luồng chính (Tạo) & 1. Nhập tiêu đề Card \newline 2. Hệ thống tạo Card trong List với \texttt{position} ở cuối \newline 3. Card hiển thị ở cuối List tương ứng \\
\hline
Luồng chính (Cập nhật) & 1. Mở chi tiết Card \newline 2. Chỉnh sửa tiêu đề, mô tả, ngày bắt đầu, ngày đến hạn, trạng thái hoàn thành \newline 3. Hệ thống cập nhật bản ghi trong bảng \texttt{cards} \\
\hline
Luồng chính (Xóa) & 1. Chọn xóa Card \newline 2. Hệ thống xóa Card và toàn bộ Checklist, Checklist Item, Card Member bên trong (cascade delete) \\
\hline
Luồng thay thế & List không tồn tại: Trả về HTTP 404 \newline Người dùng không có quyền: Trả về HTTP 403 \newline Lỗi không xác định: Trả về HTTP 500 \\
\hline
\end{tabular}
\caption{Đặc tả Use Case UC-009: Quản lý Card}
\label{tab:uc009}
\end{table}

\begin{table}[H]
\centering
\begin{tabular}{|p{4cm}|p{11cm}|}
\hline
\textbf{Thuộc tính} & \textbf{Nội dung} \\
\hline
Use Case ID & UC-010 \\
\hline
Tên Use Case & Kéo thả Card và List \\
\hline
Actor & ADMIN hoặc MEMBER Board \\
\hline
Mục tiêu & Di chuyển Card giữa các List hoặc thay đổi thứ tự Card/List trong Board \\
\hline
Tiền điều kiện & Board đang được hiển thị; người dùng có quyền ADMIN hoặc MEMBER \\
\hline
Hậu điều kiện & Trường \texttt{position} của Card hoặc List được cập nhật; giao diện phản ánh thứ tự mới \\
\hline
Luồng chính & 1. Nhấn và giữ Card hoặc List cần di chuyển \newline 2. Kéo đến vị trí đích \newline 3. Thả để xác nhận vị trí mới \newline 4. Frontend cập nhật giao diện ngay lập tức (optimistic update) \newline 5. Gửi yêu cầu \texttt{PATCH} đến Backend với giá trị \texttt{position} mới (và \texttt{listId} mới nếu di chuyển Card sang List khác) \newline 6. Backend cập nhật bản ghi trong cơ sở dữ liệu \\
\hline
Luồng thay thế & Cập nhật thất bại: Frontend hoàn tác về trạng thái ban đầu \newline Lỗi không xác định: Trả về HTTP 500 \\
\hline
\end{tabular}
\caption{Đặc tả Use Case UC-010: Kéo thả Card và List}
\label{tab:uc010}
\end{table}

\begin{table}[H]
\centering
\begin{tabular}{|p{4cm}|p{11cm}|}
\hline
\textbf{Thuộc tính} & \textbf{Nội dung} \\
\hline
Use Case ID & UC-011 \\
\hline
Tên Use Case & Quản lý Checklist \\
\hline
Actor & ADMIN hoặc MEMBER Board \\
\hline
Mục tiêu & Tạo danh sách kiểm tra và theo dõi tiến độ công việc con trong Card \\
\hline
Tiền điều kiện & Card tồn tại; người dùng có quyền ADMIN hoặc MEMBER \\
\hline
Hậu điều kiện & Checklist và các Checklist Item được tạo; thanh tiến độ phản ánh tỷ lệ hoàn thành \\
\hline
Luồng chính & 1. Mở chi tiết Card \newline 2. Tạo Checklist với tên xác định \newline 3. Thêm các Checklist Item vào Checklist \newline 4. Đánh dấu hoàn thành từng Checklist Item (\texttt{is\_completed = true}) \newline 5. Thanh tiến độ (progress bar) tự động cập nhật tỷ lệ phần trăm hoàn thành \\
\hline
Luồng thay thế & Xóa Checklist: Toàn bộ Checklist Item bên trong bị xóa theo (cascade delete) \newline Lỗi không xác định: Trả về HTTP 500 \\
\hline
\end{tabular}
\caption{Đặc tả Use Case UC-011: Quản lý Checklist}
\label{tab:uc011}
\end{table}

\begin{table}[H]
\centering
\begin{tabular}{|p{4cm}|p{11cm}|}
\hline
\textbf{Thuộc tính} & \textbf{Nội dung} \\
\hline
Use Case ID & UC-012 \\
\hline
Tên Use Case & Gán thành viên vào Card \\
\hline
Actor & ADMIN hoặc MEMBER Board \\
\hline
Mục tiêu & Phân công người chịu trách nhiệm thực hiện công việc trong Card \\
\hline
Tiền điều kiện & Card tồn tại; thành viên đã là thành viên của Board \\
\hline
Hậu điều kiện & Thành viên được thêm vào \texttt{card\_members}; avatar hiển thị trực tiếp trên Card trong Kanban view \\
\hline
Luồng chính & 1. Mở chi tiết Card \newline 2. Tìm kiếm và chọn thành viên thuộc Board \newline 3. Gửi yêu cầu gán thành viên qua \texttt{POST /api/v1/card-members} \newline 4. Hệ thống tạo bản ghi trong \texttt{card\_members} \newline 5. Avatar thành viên xuất hiện trên Card \\
\hline
Luồng thay thế & Thành viên đã được gán: Không tạo bản ghi trùng lặp (ràng buộc UNIQUE) \newline Hủy gán: \texttt{DELETE /api/v1/card-members/:id} \newline Lỗi không xác định: Trả về HTTP 500 \\
\hline
\end{tabular}
\caption{Đặc tả Use Case UC-012: Gán thành viên vào Card}
\label{tab:uc012}
\end{table}

\begin{table}[H]
\centering
\begin{tabular}{|p{4cm}|p{11cm}|}
\hline
\textbf{Thuộc tính} & \textbf{Nội dung} \\
\hline
Use Case ID & UC-013 \\
\hline
Tên Use Case & Thêm thành viên vào Board \\
\hline
Actor & ADMIN Board \\
\hline
Mục tiêu & Thêm thành viên thuộc Workspace vào Board với vai trò xác định \\
\hline
Tiền điều kiện & Người dùng có vai trò ADMIN trong Board; người được mời đã là thành viên của Workspace chứa Board \\
\hline
Hậu điều kiện & Thành viên được thêm vào \texttt{board\_members} với vai trò được chỉ định \\
\hline
Luồng chính & 1. Tìm kiếm thành viên thuộc Workspace qua \texttt{GET /api/v1/board-members?boardId=\&search=} \newline 2. Chọn thành viên và xác định vai trò (ADMIN, MEMBER, VIEWER) \newline 3. Gửi yêu cầu thêm qua \texttt{POST /api/v1/boards-members} \newline 4. Hệ thống tạo bản ghi trong \texttt{board\_members} \\
\hline
Luồng thay thế & Thành viên chưa thuộc Workspace: Không thể thêm vào Board \newline Thành viên đã trong Board: Trả về lỗi vi phạm ràng buộc UNIQUE \newline Lỗi không xác định: Trả về HTTP 500 \\
\hline
\end{tabular}
\caption{Đặc tả Use Case UC-013: Thêm thành viên vào Board}
\label{tab:uc013}
\end{table}

\subsection{Phân tích dữ liệu}

\subsubsection{Các thực thể chính}

Hệ thống xoay quanh sáu thực thể nghiệp vụ chính được tổ chức theo cấu trúc phân cấp:

\begin{itemize}
    \item \textbf{User (Người dùng):} Đại diện cho tài khoản người dùng trong hệ thống. Thông tin danh tính và xác thực được quản lý bởi Amazon Cognito; hệ thống nội bộ lưu trữ thông tin bổ sung như họ tên, ảnh đại diện và mã định danh Cognito.
    \item \textbf{Workspace (Không gian làm việc):} Đơn vị tổ chức cấp cao nhất, quản lý tập trung nhiều Board liên quan. Mỗi Workspace có một người sở hữu (owner) và nhiều thành viên.
    \item \textbf{Board (Bảng công việc):} Đại diện cho một dự án cụ thể trong Workspace. Mỗi Board chứa nhiều List và có danh sách thành viên riêng.
    \item \textbf{List (Danh sách):} Cột trạng thái trong Board, thể hiện giai đoạn thực hiện của công việc. Thứ tự các List được quản lý qua trường \texttt{position}.
    \item \textbf{Card (Thẻ công việc):} Đơn vị công việc cơ bản, chứa thông tin chi tiết về nhiệm vụ cần thực hiện. Card thuộc về một List và có thứ tự xác định bằng \texttt{position}.
    \item \textbf{Checklist:} Danh sách kiểm tra trong Card, hỗ trợ theo dõi các công việc con. Mỗi Checklist có nhiều ChecklistItem.
    \item \textbf{ChecklistItem:} Hạng mục cụ thể trong Checklist, có thể được đánh dấu hoàn thành hoặc chưa hoàn thành, và có thể thiết lập thời hạn riêng.
\end{itemize}

\subsubsection{Quan hệ giữa các thực thể}

\begin{itemize}
    \item Một \textbf{User} có thể sở hữu nhiều \textbf{Workspace} và là thành viên của nhiều \textbf{Workspace} và \textbf{Board} khác.
    \item Một \textbf{Workspace} chứa nhiều \textbf{Board} và có nhiều \textbf{User} làm thành viên (quan hệ nhiều-nhiều qua bảng \texttt{workspace\_member}).
    \item Một \textbf{Board} chứa nhiều \textbf{List} và có nhiều \textbf{User} làm thành viên với các vai trò khác nhau (quan hệ nhiều-nhiều qua bảng \texttt{board\_member}).
    \item Một \textbf{List} chứa nhiều \textbf{Card}; các Card được sắp xếp theo thứ tự \texttt{position}.
    \item Một \textbf{Card} có thể chứa nhiều \textbf{Checklist} và được gán cho nhiều \textbf{User} (quan hệ nhiều-nhiều qua bảng \texttt{card\_member}).
    \item Một \textbf{Checklist} chứa nhiều \textbf{ChecklistItem}.
\end{itemize}

\subsection{Luồng dữ liệu}

\subsubsection{Luồng xác thực người dùng}

Luồng xác thực được thiết kế theo mô hình ủy quyền cho Amazon Cognito. Backend đóng vai trò trung gian giữa Frontend và Cognito, đảm bảo người dùng không bao giờ giao tiếp trực tiếp với dịch vụ định danh.

\textbf{Luồng đăng ký và xác nhận tài khoản:}
\begin{enumerate}
    \item Người dùng nhập họ tên, email, mật khẩu trên Frontend.
    \item Frontend gửi \texttt{POST /api/v1/auth/sign-up} đến Backend.
    \item Backend gọi Cognito \texttt{signUp()} với thông tin đăng ký.
    \item Cognito tạo tài khoản và gửi mã OTP qua email.
    \item Người dùng nhập mã OTP, Frontend gửi \texttt{POST /api/v1/auth/confirm-sign-up}.
    \item Backend gọi Cognito \texttt{confirmSignUp()}, tài khoản được kích hoạt.
\end{enumerate}

\textbf{Luồng đăng nhập:}
\begin{enumerate}
    \item Người dùng nhập email và mật khẩu, Frontend gửi \texttt{POST /api/v1/auth/sign-in}.
    \item Backend gọi Cognito \texttt{initiateAuth()} với phương thức \texttt{USER\_PASSWORD\_AUTH}.
    \item Cognito xác thực và trả về access token, refresh token.
    \item Backend đặt hai HttpOnly Cookie (\texttt{ACCESS\_TOKEN}, \texttt{REFRESH\_TOKEN}) vào response.
    \item Frontend điều hướng người dùng về Dashboard.
    \item Mỗi request tiếp theo của Frontend đều gửi kèm Cookie; Backend xác thực JWT trước khi xử lý.
\end{enumerate}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{./images/chap3/Auth Flow.png}
    \caption{Luồng xác thực của hệ thống Nello}
    \label{fig:auth_flow}
\end{figure}

\subsubsection{Luồng tạo và cập nhật Card}

Luồng tạo Card là đại diện điển hình cho các luồng nghiệp vụ CRUD trong hệ thống, thể hiện cách Backend kiểm tra quyền truy cập trước khi thực hiện thao tác với cơ sở dữ liệu.

\begin{enumerate}
    \item Người dùng nhập tiêu đề Card trong giao diện Kanban, Frontend gửi \texttt{POST /api/v1/cards} với \texttt{listId}, \texttt{title}, \texttt{position}.
    \item Backend kiểm tra tính hợp lệ của JWT từ HttpOnly Cookie.
    \item Backend kiểm tra List tồn tại; nếu không tìm thấy, trả về HTTP 404.
    \item Backend kiểm tra quyền của người dùng trong Board chứa List; nếu không đủ quyền, trả về HTTP 403.
    \item Backend thực hiện \texttt{INSERT} vào bảng \texttt{cards}.
    \item Backend trả về HTTP 201 với thông tin Card vừa tạo (bao gồm \texttt{cardId}, \texttt{title}, \texttt{listId}, \texttt{position}).
    \item Frontend cập nhật giao diện, hiển thị Card mới ở vị trí đã xác định trong List.
\end{enumerate}

\subsubsection{Luồng kéo thả Card}

Tính năng kéo thả áp dụng kỹ thuật cập nhật lạc quan (optimistic update) trên Frontend để đảm bảo trải nghiệm người dùng mượt mà, đồng thời đồng bộ với Backend bằng thuật toán fractional indexing trên trường \texttt{position} kiểu TEXT.

\begin{enumerate}
    \item Người dùng kéo Card từ vị trí nguồn đến vị trí đích (có thể trong cùng List hoặc sang List khác).
    \item Frontend ngay lập tức cập nhật thứ tự hiển thị (optimistic update), không chờ phản hồi từ Backend.
    \item Frontend tính toán giá trị \texttt{position} mới là chuỗi ký tự nằm giữa hai phần tử lân cận tại vị trí đích.
    \item Frontend gửi \texttt{PATCH /api/v1/cards/:id} với \texttt{position} mới và \texttt{listId} đích (nếu di chuyển sang List khác).
    \item Backend cập nhật bản ghi trong bảng \texttt{cards}.
    \item Nếu Backend trả về lỗi, Frontend hoàn tác (rollback) giao diện về trạng thái ban đầu.
\end{enumerate}

\subsection{Thiết kế cơ sở dữ liệu}

Cơ sở dữ liệu PostgreSQL được thiết kế bao gồm các kiểu liệt kê (ENUM), bảng tra cứu và bảng nghiệp vụ. Mỗi bảng được mô tả chi tiết qua cấu trúc cột, kiểu dữ liệu và ràng buộc tương ứng.

\subsubsection{Kiểu liệt kê}

Hệ thống định nghĩa kiểu liệt kê \texttt{role\_name\_enum} để chuẩn hóa các vai trò người dùng trong toàn bộ hệ thống.

\begin{table}[H]
\centering
\begin{tabular}{|l|p{10cm}|}
\hline
\textbf{Giá trị} & \textbf{Mô tả} \\
\hline
\texttt{ADMIN} & Quản trị viên, có toàn quyền quản lý trong Workspace hoặc Board. \\
\hline
\texttt{MEMBER} & Thành viên thông thường. \\
\hline
\texttt{VIEWER} & Người xem, chỉ có quyền đọc dữ liệu. \\
\hline
\end{tabular}
\caption{Kiểu liệt kê \texttt{role\_name\_enum}}
\label{tab:enum_role}
\end{table}

\subsubsection{Bảng tra cứu}

\begin{table}[H]
\centering
\begin{tabular}{|p{3cm}|p{3cm}|p{3.5cm}|p{5cm}|}
\hline
\textbf{Tên cột} & \textbf{Kiểu dữ liệu} & \textbf{Ràng buộc} & \textbf{Mô tả} \\
\hline
\texttt{id} & INT & Khóa chính, NOT NULL & Mã vai trò. \\
\hline
\texttt{name} & \texttt{role\_name\_enum} & NOT NULL, UNIQUE & Tên vai trò (ADMIN, MEMBER, VIEWER). \\
\hline
\end{tabular}
\caption{Bảng \texttt{roles}}
\label{tab:roles}
\end{table}

\begin{table}[H]
\centering
\begin{tabular}{|p{3cm}|p{3cm}|p{3.5cm}|p{5cm}|}
\hline
\textbf{Tên cột} & \textbf{Kiểu dữ liệu} & \textbf{Ràng buộc} & \textbf{Mô tả} \\
\hline
\texttt{id} & INT & Khóa chính, NOT NULL & Mã hình ảnh. \\
\hline
\texttt{key} & TEXT & NOT NULL & Đường dẫn tương đối đến tệp hình nền Board. \\
\hline
\end{tabular}
\caption{Bảng \texttt{images}}
\label{tab:images}
\end{table}

\begin{table}[H]
\centering
\begin{tabular}{|p{3cm}|p{3cm}|p{3.5cm}|p{5cm}|}
\hline
\textbf{Tên cột} & \textbf{Kiểu dữ liệu} & \textbf{Ràng buộc} & \textbf{Mô tả} \\
\hline
\texttt{id} & INT & Khóa chính & Mã loại Workspace. \\
\hline
\texttt{name} & VARCHAR(255) & NOT NULL & Tên loại Workspace (ví dụ: Sales CRM, IT, Education...). \\
\hline
\end{tabular}
\caption{Bảng \texttt{workspace\_categories}}
\label{tab:workspace_categories}
\end{table}

\subsubsection{Bảng nghiệp vụ}

\begin{table}[H]
\centering
\begin{tabular}{|p{3cm}|p{3cm}|p{3.5cm}|p{5cm}|}
\hline
\textbf{Tên cột} & \textbf{Kiểu dữ liệu} & \textbf{Ràng buộc} & \textbf{Mô tả} \\
\hline
\texttt{id} & UUID & Khóa chính, NOT NULL & Mã định danh nội bộ. \\
\hline
\texttt{cognito\_id} & VARCHAR(255) & NOT NULL, UNIQUE & Mã định danh từ Amazon Cognito. \\
\hline
\texttt{full\_name} & VARCHAR(255) & NOT NULL & Họ tên đầy đủ. \\
\hline
\texttt{email} & VARCHAR(255) & NOT NULL, UNIQUE & Địa chỉ email. \\
\hline
\texttt{avatar\_url} & VARCHAR(255) & NULL & Đường dẫn ảnh đại diện lưu trên S3. \\
\hline
\texttt{bio} & TEXT & NULL & Giới thiệu bản thân. \\
\hline
\texttt{phone} & VARCHAR(255) & NULL & Số điện thoại. \\
\hline
\texttt{address} & VARCHAR(255) & NULL & Địa chỉ. \\
\hline
\texttt{deleted\_at} & TIMESTAMP & NULL & Thời điểm xóa mềm tài khoản. \\
\hline
\texttt{created\_at} & TIMESTAMP & NOT NULL, DEFAULT NOW() & Thời điểm tạo bản ghi. \\
\hline
\texttt{updated\_at} & TIMESTAMP & NOT NULL, DEFAULT NOW() & Thời điểm cập nhật gần nhất. \\
\hline
\end{tabular}
\caption{Bảng \texttt{users}}
\label{tab:users}
\end{table}

\begin{table}[H]
\centering
\begin{tabular}{|p{3cm}|p{3cm}|p{3.5cm}|p{5cm}|}
\hline
\textbf{Tên cột} & \textbf{Kiểu dữ liệu} & \textbf{Ràng buộc} & \textbf{Mô tả} \\
\hline
\texttt{id} & UUID & Khóa chính, NOT NULL & Mã định danh. \\
\hline
\texttt{name} & VARCHAR(255) & NOT NULL & Tên Workspace. \\
\hline
\texttt{description} & VARCHAR(255) & NULL & Mô tả Workspace. \\
\hline
\texttt{category\_id} & INT & NOT NULL, FK $\rightarrow$ \texttt{workspace\_categories} & Loại Workspace. \\
\hline
\texttt{created\_by} & UUID & NOT NULL, FK $\rightarrow$ \texttt{users} & Người tạo Workspace. \\
\hline
\texttt{updated\_by} & UUID & NULL, FK $\rightarrow$ \texttt{users} & Người cập nhật gần nhất. \\
\hline
\texttt{created\_at} & TIMESTAMP & NOT NULL, DEFAULT NOW() & Thời điểm tạo bản ghi. \\
\hline
\texttt{updated\_at} & TIMESTAMP & NOT NULL, DEFAULT NOW() & Thời điểm cập nhật gần nhất. \\
\hline
\end{tabular}
\caption{Bảng \texttt{workspaces}}
\label{tab:workspaces}
\end{table}

\begin{table}[H]
\centering
\begin{tabular}{|p{3cm}|p{3cm}|p{3.5cm}|p{5cm}|}
\hline
\textbf{Tên cột} & \textbf{Kiểu dữ liệu} & \textbf{Ràng buộc} & \textbf{Mô tả} \\
\hline
\texttt{id} & UUID & Khóa chính, NOT NULL & Mã định danh. \\
\hline
\texttt{user\_id} & UUID & NOT NULL, FK $\rightarrow$ \texttt{users} & Thành viên được thêm vào Workspace. \\
\hline
\texttt{workspace\_id} & UUID & NOT NULL, FK $\rightarrow$ \texttt{workspaces} & Workspace chứa thành viên. \\
\hline
\texttt{role\_id} & INT & NOT NULL, FK $\rightarrow$ \texttt{roles} & Vai trò của thành viên trong Workspace. \\
\hline
\texttt{created\_by} & UUID & NOT NULL, FK $\rightarrow$ \texttt{users} & Người thêm thành viên. \\
\hline
\texttt{created\_at} & TIMESTAMP & NOT NULL, DEFAULT NOW() & Thời điểm thêm thành viên. \\
\hline
\multicolumn{4}{|l|}{Ràng buộc UNIQUE: (\texttt{user\_id}, \texttt{workspace\_id})} \\
\hline
\end{tabular}
\caption{Bảng \texttt{workspace\_members} -- quan hệ nhiều-nhiều giữa \texttt{users} và \texttt{workspaces}}
\label{tab:workspace_members}
\end{table}

\begin{table}[H]
\centering
\begin{tabular}{|p{3cm}|p{3cm}|p{3.5cm}|p{5cm}|}
\hline
\textbf{Tên cột} & \textbf{Kiểu dữ liệu} & \textbf{Ràng buộc} & \textbf{Mô tả} \\
\hline
\texttt{id} & UUID & Khóa chính, NOT NULL & Mã định danh. \\
\hline
\texttt{name} & VARCHAR(255) & NOT NULL & Tên Board. \\
\hline
\texttt{background\_url} & TEXT & NULL & Đường dẫn hình nền. \\
\hline
\texttt{workspace\_id} & UUID & NOT NULL, FK $\rightarrow$ \texttt{workspaces} & Workspace chứa Board. \\
\hline
\texttt{created\_by} & UUID & NOT NULL, FK $\rightarrow$ \texttt{users} & Người tạo Board. \\
\hline
\texttt{updated\_by} & UUID & NULL, FK $\rightarrow$ \texttt{users} & Người cập nhật gần nhất. \\
\hline
\texttt{created\_at} & TIMESTAMP & NOT NULL, DEFAULT NOW() & Thời điểm tạo bản ghi. \\
\hline
\texttt{updated\_at} & TIMESTAMP & NOT NULL, DEFAULT NOW() & Thời điểm cập nhật gần nhất. \\
\hline
\end{tabular}
\caption{Bảng \texttt{boards}}
\label{tab:boards}
\end{table}

\begin{table}[H]
\centering
\begin{tabular}{|p{3cm}|p{3cm}|p{3.5cm}|p{5cm}|}
\hline
\textbf{Tên cột} & \textbf{Kiểu dữ liệu} & \textbf{Ràng buộc} & \textbf{Mô tả} \\
\hline
\texttt{id} & UUID & Khóa chính, NOT NULL & Mã định danh. \\
\hline
\texttt{user\_id} & UUID & NOT NULL, FK $\rightarrow$ \texttt{users} & Thành viên được thêm vào Board. \\
\hline
\texttt{board\_id} & UUID & NOT NULL, FK $\rightarrow$ \texttt{boards} & Board chứa thành viên. \\
\hline
\texttt{role\_id} & INT & NOT NULL, FK $\rightarrow$ \texttt{roles} & Vai trò trong Board (ADMIN, MEMBER, VIEWER). \\
\hline
\texttt{created\_by} & UUID & NOT NULL, FK $\rightarrow$ \texttt{users} & Người thêm thành viên. \\
\hline
\texttt{created\_at} & TIMESTAMP & NOT NULL, DEFAULT NOW() & Thời điểm thêm thành viên. \\
\hline
\multicolumn{4}{|l|}{Ràng buộc UNIQUE: (\texttt{user\_id}, \texttt{board\_id})} \\
\hline
\end{tabular}
\caption{Bảng \texttt{board\_members} -- quan hệ nhiều-nhiều giữa \texttt{users} và \texttt{boards}}
\label{tab:board_members}
\end{table}

\begin{table}[H]
\centering
\begin{tabular}{|p{3cm}|p{3cm}|p{3.5cm}|p{5cm}|}
\hline
\textbf{Tên cột} & \textbf{Kiểu dữ liệu} & \textbf{Ràng buộc} & \textbf{Mô tả} \\
\hline
\texttt{id} & UUID & Khóa chính, NOT NULL & Mã định danh. \\
\hline
\texttt{name} & VARCHAR(255) & NOT NULL & Tên List. \\
\hline
\texttt{position} & TEXT & NOT NULL & Giá trị vị trí dùng để sắp xếp và xử lý kéo thả. \\
\hline
\texttt{board\_id} & UUID & NOT NULL, FK $\rightarrow$ \texttt{boards} & Board chứa List. \\
\hline
\texttt{created\_by} & UUID & NOT NULL, FK $\rightarrow$ \texttt{users} & Người tạo List. \\
\hline
\texttt{updated\_by} & UUID & NULL, FK $\rightarrow$ \texttt{users} & Người cập nhật gần nhất. \\
\hline
\texttt{created\_at} & TIMESTAMP & NOT NULL, DEFAULT NOW() & Thời điểm tạo bản ghi. \\
\hline
\texttt{updated\_at} & TIMESTAMP & NOT NULL, DEFAULT NOW() & Thời điểm cập nhật gần nhất. \\
\hline
\end{tabular}
\caption{Bảng \texttt{lists}}
\label{tab:lists}
\end{table}

\begin{table}[H]
\centering
\begin{tabular}{|p{3cm}|p{3cm}|p{3.5cm}|p{5cm}|}
\hline
\textbf{Tên cột} & \textbf{Kiểu dữ liệu} & \textbf{Ràng buộc} & \textbf{Mô tả} \\
\hline
\texttt{id} & UUID & Khóa chính, NOT NULL & Mã định danh. \\
\hline
\texttt{title} & VARCHAR(255) & NOT NULL & Tiêu đề Card. \\
\hline
\texttt{description} & TEXT & NULL & Mô tả chi tiết. \\
\hline
\texttt{position} & TEXT & NOT NULL & Giá trị vị trí dùng để sắp xếp và xử lý kéo thả. \\
\hline
\texttt{list\_id} & UUID & NOT NULL, FK $\rightarrow$ \texttt{lists} & List chứa Card. \\
\hline
\texttt{start\_date} & TIMESTAMP & NULL & Ngày bắt đầu. \\
\hline
\texttt{due\_date} & TIMESTAMP & NULL & Ngày đến hạn. \\
\hline
\texttt{is\_completed} & BOOLEAN & DEFAULT FALSE & Trạng thái hoàn thành của Card. \\
\hline
\texttt{created\_by} & UUID & NOT NULL, FK $\rightarrow$ \texttt{users} & Người tạo Card. \\
\hline
\texttt{updated\_by} & UUID & NULL, FK $\rightarrow$ \texttt{users} & Người cập nhật gần nhất. \\
\hline
\texttt{created\_at} & TIMESTAMP & NOT NULL, DEFAULT NOW() & Thời điểm tạo bản ghi. \\
\hline
\texttt{updated\_at} & TIMESTAMP & NOT NULL, DEFAULT NOW() & Thời điểm cập nhật gần nhất. \\
\hline
\end{tabular}
\caption{Bảng \texttt{cards}}
\label{tab:cards}
\end{table}

\begin{table}[H]
\centering
\begin{tabular}{|p{3cm}|p{3cm}|p{3.5cm}|p{5cm}|}
\hline
\textbf{Tên cột} & \textbf{Kiểu dữ liệu} & \textbf{Ràng buộc} & \textbf{Mô tả} \\
\hline
\texttt{id} & UUID & Khóa chính, NOT NULL & Mã định danh. \\
\hline
\texttt{user\_id} & UUID & NOT NULL, FK $\rightarrow$ \texttt{users} & Thành viên được gán vào Card. \\
\hline
\texttt{card\_id} & UUID & NOT NULL, FK $\rightarrow$ \texttt{cards} & Card được gán thành viên. \\
\hline
\texttt{created\_by} & UUID & NOT NULL, FK $\rightarrow$ \texttt{users} & Người gán thành viên. \\
\hline
\texttt{created\_at} & TIMESTAMP & NOT NULL, DEFAULT NOW() & Thời điểm gán thành viên. \\
\hline
\multicolumn{4}{|l|}{Ràng buộc UNIQUE: (\texttt{user\_id}, \texttt{card\_id})} \\
\hline
\end{tabular}
\caption{Bảng \texttt{card\_members} -- quan hệ nhiều-nhiều giữa \texttt{users} và \texttt{cards}}
\label{tab:card_members}
\end{table}

\begin{table}[H]
\centering
\begin{tabular}{|p{3cm}|p{3cm}|p{3.5cm}|p{5cm}|}
\hline
\textbf{Tên cột} & \textbf{Kiểu dữ liệu} & \textbf{Ràng buộc} & \textbf{Mô tả} \\
\hline
\texttt{id} & UUID & Khóa chính, NOT NULL & Mã định danh. \\
\hline
\texttt{name} & VARCHAR(255) & NOT NULL & Tên Checklist. \\
\hline
\texttt{position} & TEXT & NOT NULL & Vị trí hiển thị trong Card. \\
\hline
\texttt{card\_id} & UUID & NOT NULL, FK $\rightarrow$ \texttt{cards} & Card chứa Checklist. \\
\hline
\texttt{created\_by} & UUID & NOT NULL, FK $\rightarrow$ \texttt{users} & Người tạo Checklist. \\
\hline
\texttt{updated\_by} & UUID & NULL, FK $\rightarrow$ \texttt{users} & Người cập nhật gần nhất. \\
\hline
\texttt{created\_at} & TIMESTAMP & NOT NULL, DEFAULT NOW() & Thời điểm tạo bản ghi. \\
\hline
\texttt{updated\_at} & TIMESTAMP & NOT NULL, DEFAULT NOW() & Thời điểm cập nhật gần nhất. \\
\hline
\end{tabular}
\caption{Bảng \texttt{checklists}}
\label{tab:checklists}
\end{table}

\begin{table}[H]
\centering
\begin{tabular}{|p{3cm}|p{3cm}|p{3.5cm}|p{5cm}|}
\hline
\textbf{Tên cột} & \textbf{Kiểu dữ liệu} & \textbf{Ràng buộc} & \textbf{Mô tả} \\
\hline
\texttt{id} & UUID & Khóa chính, NOT NULL & Mã định danh. \\
\hline
\texttt{name} & VARCHAR(255) & NOT NULL & Tên hạng mục công việc con. \\
\hline
\texttt{position} & TEXT & NOT NULL & Vị trí hiển thị trong Checklist. \\
\hline
\texttt{is\_completed} & BOOLEAN & NOT NULL, DEFAULT FALSE & Trạng thái hoàn thành của hạng mục. \\
\hline
\texttt{due\_date} & TIMESTAMP & NULL & Thời hạn riêng của hạng mục. \\
\hline
\texttt{checklist\_id} & UUID & NOT NULL, FK $\rightarrow$ \texttt{checklists} & Checklist chứa hạng mục. \\
\hline
\texttt{created\_by} & UUID & NOT NULL, FK $\rightarrow$ \texttt{users} & Người tạo hạng mục. \\
\hline
\texttt{updated\_by} & UUID & NULL, FK $\rightarrow$ \texttt{users} & Người cập nhật gần nhất. \\
\hline
\texttt{created\_at} & TIMESTAMP & NOT NULL, DEFAULT NOW() & Thời điểm tạo bản ghi. \\
\hline
\texttt{updated\_at} & TIMESTAMP & NOT NULL, DEFAULT NOW() & Thời điểm cập nhật gần nhất. \\
\hline
\multicolumn{4}{|l|}{Ràng buộc CHECK: \texttt{is\_completed IN (true, false)}} \\
\hline
\end{tabular}
\caption{Bảng \texttt{checklist\_items}}
\label{tab:checklist_items}
\end{table}

\subsection{Thiết kế tổng thể hệ thống}

Hệ thống được triển khai trên nền tảng điện toán đám mây AWS với sự bao bọc trong một Mạng riêng ảo (VPC) nhằm tối ưu hóa tính bảo mật. Kiến trúc tổng thể được thiết kế với 9 thành phần chính và sự tương tác chặt chẽ như sau:

\begin{itemize}
    \item \textbf{Client:} Đại diện cho trình duyệt web của người dùng cuối, là nơi giao diện ứng dụng được hiển thị và thực hiện các tương tác với hệ thống.

    \item \textbf{Amazon CloudFront:} Đóng vai trò là mạng phân phối nội dung (CDN). CloudFront \textit{chỉ tương tác trực tiếp với Client và Amazon S3}, có nhiệm vụ tăng tốc độ tải các tệp tin tĩnh (hình ảnh, tài liệu) từ S3 đến người dùng một cách tối ưu thông qua bộ nhớ đệm (caching), giảm chi phí sử dụng S3.

    \item \textbf{Amazon S3:} Được sử dụng chuyên biệt làm kho lưu trữ các tệp đa phương tiện do người dùng tải lên (ví dụ: avatar, tệp đính kèm trong thẻ công việc). Tầng Backend đảm nhận hoàn toàn việc tải tệp tin lên S3 bằng cách tương tác trực tiếp với dịch vụ này; Client không có quyền đẩy trực tiếp tệp tin lên S3.

    \item \textbf{Amazon VPC (Virtual Private Cloud):} Cung cấp một môi trường mạng nội bộ bị cách ly với Internet. Toàn bộ các máy chủ EC2 (cả Frontend và Backend) cũng như cơ sở dữ liệu RDS đều được đặt bên trong VPC này (phân vùng private).

    \item \textbf{Amazon ALB (Application Load Balancer):} Là điểm truy cập (entry point) công khai duy nhất nhận lưu lượng truy cập từ Internet vào VPC. ALB tiếp nhận mọi request từ Client và phân phối tải vào các máy chủ EC2 (Frontend hoặc Backend) ở bên trong mạng private.

    \item \textbf{EC2 (Frontend – Nginx + Docker + React):} Ứng dụng ReactJS được đóng gói bằng Docker và triển khai thông qua web server Nginx trên máy chủ ảo EC2. Máy chủ này là một EC2 private, chỉ nhận lưu lượng HTTP/HTTPS từ ALB chuyển vào thay vì tiếp xúc trực tiếp với Internet.

    \item \textbf{EC2 (Backend – Spring Boot):} Máy chủ chạy ứng dụng API Spring Boot, xử lý các logic nghiệp vụ. Tương tự như Frontend, máy chủ này cũng là một EC2 private và chỉ nhận request nội bộ từ ALB.

    \item \textbf{Amazon Cognito:} Dịch vụ quản lý định danh. Trong kiến trúc này, Cognito chỉ tương tác trực tiếp với tầng Backend. Thay vì Client gọi trực tiếp Cognito, Backend sẽ chịu trách nhiệm gọi API của Cognito để thực hiện xác thực người dùng và nhận JWT.

    \item \textbf{Amazon RDS (PostgreSQL):} Hệ quản trị cơ sở dữ liệu quan hệ được triển khai trong phân vùng mạng nội bộ sâu nhất. Chỉ duy nhất tầng Backend (EC2 chạy Spring Boot) mới được phép tương tác trực tiếp với RDS để thực hiện truy xuất và lưu trữ dữ liệu; mọi nỗ lực truy cập từ các thành phần khác đều bị từ chối hoàn toàn.
\end{itemize}

Luồng hoạt động tổng thể của hệ thống diễn ra như sau:
\begin{enumerate}
    \item \textbf{Tải giao diện ứng dụng:} Client gửi request $\rightarrow$ ALB $\rightarrow$ EC2 (Frontend – Nginx + React) $\rightarrow$ Trả về giao diện cho người dùng.
    \item \textbf{Tải tài nguyên tĩnh (hình ảnh, tệp):} Client $\rightarrow$ CloudFront $\rightarrow$ S3 $\rightarrow$ Trả về Client.
    \item \textbf{Xử lý nghiệp vụ và xác thực:} Client gọi API $\rightarrow$ ALB $\rightarrow$ EC2 (Backend – Spring Boot). Tại đây, nếu là tác vụ đăng nhập hoặc đăng ký, EC2 Backend sẽ giao tiếp với Cognito. Đối với các tác vụ liên quan đến dữ liệu hệ thống, EC2 Backend sẽ giao tiếp trực tiếp với RDS (PostgreSQL). Kết quả xử lý cuối cùng được Backend trả về qua ALB đến Client.
\end{enumerate}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{"./images/chap2/Nello System Architech.png"}
    \caption{Kiến trúc tổng thể của hệ thống Nello}
    \label{fig:nello_system_architech_chap3}
\end{figure}

\clearpage

\section{CHƯƠNG 4: TRIỂN KHAI VÀ KẾT QUẢ}

\subsection{Các chức năng đã hoàn thành}

Sau 8 tuần thực hiện (từ 30/03 đến 24/05/2026), hệ thống quản lý công việc Nello đã hoàn thiện triển khai trên cả tầng Backend và Frontend cho phần lớn các Use Case đã đặc tả ở Chương 3. Nội dung dưới đây trình bày mức độ hoàn thành theo từng Use Case, bao gồm trạng thái Backend, trạng thái Frontend và tỷ lệ hoàn thành tổng thể.

\subsubsection{Tổng hợp mức độ hoàn thành theo Use Case}

\begin{table}[H]
\centering
\begin{tabular}{|p{1.8cm}|p{5.5cm}|p{2cm}|p{2cm}|p{2.2cm}|}
\hline
\textbf{UC ID} & \textbf{Tên Use Case} & \textbf{Backend} & \textbf{Frontend} & \textbf{Hoàn thành} \\
\hline
UC-001 & Đăng ký tài khoản & 100\% & 100\% & \textbf{100\%} \\
\hline
UC-002 & Xác nhận tài khoản & 100\% & 100\% & \textbf{100\%} \\
\hline
UC-003 & Đăng nhập & 100\% & 100\% & \textbf{100\%} \\
\hline
UC-004 & Cập nhật thông tin cá nhân & 100\% & 100\% & \textbf{100\%} \\
\hline
UC-005 & Quản lý Workspace & 50\% & 30\% & \textbf{30\%} \\
\hline
UC-006 & Mời thành viên vào Workspace & 100\% & 100\% & \textbf{100\%} \\
\hline
UC-007 & Quản lý Board & 50\% & 30\% & \textbf{30\%} \\
\hline
UC-008 & Quản lý List & 100\% & 100\% & \textbf{100\%} \\
\hline
UC-009 & Quản lý Card & 100\% & 100\% & \textbf{100\%} \\
\hline
UC-010 & Kéo thả Card và List & 100\% & 70\% & \textbf{80\%} \\
\hline
UC-011 & Quản lý Checklist & 100\% & 100\% & \textbf{100\%} \\
\hline
UC-012 & Gán thành viên vào Card & 100\% & 100\% & \textbf{100\%} \\
\hline
UC-013 & Thêm thành viên vào Board & 100\% & 100\% & \textbf{100\%} \\
\hline
\multicolumn{4}{|l|}{\textbf{Tỷ lệ hoàn thành trung bình}} & \textbf{$\approx$95\%} \\
\hline
\end{tabular}
\caption{Mức độ hoàn thành triển khai theo Use Case}
\label{tab:uc_completion}
\end{table}

\subsubsection{Chi tiết triển khai theo Use Case}

\textbf{UC-001 — Đăng ký tài khoản (100\%)}

Backend triển khai endpoint \texttt{POST /api/v1/auth/sign-up} gọi Cognito \texttt{signUp()}, tạo tài khoản và kích hoạt gửi OTP qua email. Frontend cung cấp form nhập họ tên, email, mật khẩu với validation phía client, hiển thị thông báo lỗi khi tài khoản đã tồn tại và tự động điều hướng sang màn hình xác nhận OTP.

\textbf{UC-002 — Xác nhận tài khoản (100\%)}

Backend triển khai endpoint \texttt{POST /api/v1/auth/confirm-sign-up} gọi Cognito \texttt{confirmSignUp()} và endpoint \texttt{POST /api/v1/auth/resend-code} để gửi lại mã OTP. Frontend hiển thị ô nhập mã OTP 6 chữ số kèm nút gửi lại có bộ đếm thời gian (countdown timer), hiển thị thông báo lỗi khi mã không hợp lệ.

\textbf{UC-003 — Đăng nhập (100\%)}

Backend triển khai endpoint \texttt{POST /api/v1/auth/sign-in} gọi Cognito \texttt{initiateAuth()} và endpoint \texttt{POST /api/v1/auth/refresh-token} để tự động gia hạn phiên. JWT được lưu trong hai HttpOnly Cookie. Frontend xử lý tự động làm mới token khi phiên hết hạn (interceptor) và điều hướng về Dashboard sau khi đăng nhập thành công.

\textbf{UC-004 — Cập nhật thông tin cá nhân (100\%)}

Backend triển khai đầy đủ hai endpoint: \texttt{GET /api/v1/users/me} để tải thông tin người dùng hiện tại và \texttt{PUT /api/v1/users} nhận \texttt{UpdateProfileRequest} (gồm \texttt{fullName}, \texttt{avatarUrl}, \texttt{bio}, \texttt{phone}, \texttt{address}) để cập nhật hồ sơ. Frontend cung cấp giao diện chỉnh sửa thông tin cá nhân, hiển thị dữ liệu hiện tại và lưu thay đổi thông qua endpoint trên.

\textbf{UC-005 — Quản lý Workspace (30\%)}

Backend đã triển khai các endpoint đọc dữ liệu: \texttt{GET /api/v1/workspaces} (danh sách), \texttt{GET /api/v1/workspaces/:id} (chi tiết), \texttt{GET /api/v1/workspaces/:id/boards} (danh sách Board) và \texttt{GET /api/v1/workspaces/:id/members} (danh sách thành viên). Endpoint tạo Workspace (\texttt{POST}) cũng đã hoàn thành ở tầng Backend. Tuy nhiên, các endpoint cập nhật và xóa Workspace chưa được xây dựng. Về phía Frontend, chỉ hoàn thành phần hiển thị danh sách Workspace trong sidebar và trang chi tiết Workspace; các thao tác cập nhật tên, mô tả và xóa Workspace chưa được tích hợp vào giao diện. Tỷ lệ 30\% phản ánh việc chức năng đọc dữ liệu đã hoạt động, trong khi phần lớn nghiệp vụ quản lý (cập nhật, xóa) chưa được triển khai.

\textbf{UC-006 — Mời thành viên vào Workspace (100\%)}

Backend triển khai \texttt{POST /api/v1/workspaces-members/:workspaceId} để thêm thành viên với vai trò MEMBER. Frontend tích hợp modal tìm kiếm người dùng theo email thông qua \texttt{GET /api/v1/users?search=} và hiển thị danh sách thành viên kèm vai trò trong màn hình chi tiết Workspace.

\textbf{UC-007 — Quản lý Board (30\%)}

Backend đã triển khai endpoint đọc dữ liệu \texttt{GET /api/v1/boards/:id} trả về chi tiết Board kèm toàn bộ List và Card sắp xếp theo \texttt{position}, cùng với endpoint tạo Board (\texttt{POST /api/v1/boards}). Các endpoint cập nhật (\texttt{PUT /api/v1/boards/:id}) và xóa (\texttt{DELETE /api/v1/boards/:id}) đã được xây dựng ở tầng Backend nhưng chưa được tích hợp vào giao diện Frontend. Về phía Frontend, chỉ hoàn thành phần hiển thị Board theo mô hình Kanban (đọc dữ liệu) và nút tạo Board mới với modal chọn ảnh nền; các thao tác cập nhật tên Board và xóa Board chưa có trên giao diện. Tỷ lệ 30\% phản ánh việc chức năng xem dữ liệu Board hoạt động trong khi phần lớn nghiệp vụ quản lý chưa hoàn chỉnh.

\textbf{UC-008 — Quản lý List (100\%)}

Backend triển khai đầy đủ CRUD: \texttt{POST /api/v1/lists} tạo List, \texttt{PATCH /api/v1/lists/:id} cập nhật tên và vị trí, \texttt{DELETE /api/v1/lists/:id} xóa List (cascade xóa toàn bộ Card bên trong). Frontend hỗ trợ form inline tạo List, nhấp trực tiếp vào tiêu đề để đổi tên và modal xác nhận khi xóa.

\textbf{UC-009 — Quản lý Card (100\%)}

Backend triển khai đầy đủ CRUD: \texttt{POST /api/v1/cards} tạo Card, \texttt{GET /api/v1/cards/:id} lấy chi tiết, \texttt{PATCH /api/v1/cards/:id} cập nhật (tiêu đề, mô tả, ngày bắt đầu, ngày đến hạn, trạng thái hoàn thành, vị trí, List đích), \texttt{DELETE /api/v1/cards/:id} xóa Card. Frontend hiển thị modal chi tiết Card đầy đủ thông tin, badge cảnh báo Due Date (màu vàng khi sắp đến hạn, màu đỏ khi quá hạn) trực tiếp trên Card trong Kanban view.

\textbf{UC-010 — Kéo thả Card và List (80\%)}

Backend hỗ trợ hoàn toàn thông qua trường \texttt{position} kiểu TEXT và các endpoint \texttt{PATCH} cho Card và List. Frontend triển khai tính năng kéo thả với cơ chế optimistic update (cập nhật giao diện ngay trước khi nhận phản hồi từ Backend). Tuy nhiên, thuật toán tính toán vị trí dựa trên \texttt{beforeId}/\texttt{afterId} chưa xử lý đầy đủ các trường hợp biên: kéo Card hoặc List vào vị trí đầu tiên (không có \texttt{beforeId}) hoặc vị trí cuối cùng (không có \texttt{afterId}), dẫn đến sai lệch thứ tự hiển thị sau một số thao tác kéo thả liên tiếp. Tỷ lệ hoàn thành Frontend được đánh giá ở mức 70\% do lỗi edge case chưa được khắc phục.

\textbf{UC-011 — Quản lý Checklist (100\%)}

Backend triển khai đầy đủ CRUD cho cả Checklist (\texttt{/api/v1/checklists}) và ChecklistItem (\texttt{/api/v1/checklist-items}), bao gồm trường \texttt{is\_completed} và \texttt{due\_date} cho từng hạng mục. Frontend hiển thị danh sách Checklist trong modal Card Detail với thanh tiến độ (progress bar) tự động cập nhật tỷ lệ phần trăm hoàn thành theo số lượng ChecklistItem được đánh dấu.

\textbf{UC-012 — Gán thành viên vào Card (100\%)}

Backend triển khai \texttt{POST /api/v1/card-members} để gán và \texttt{DELETE /api/v1/card-members/:id} để hủy gán thành viên, với ràng buộc UNIQUE đảm bảo mỗi thành viên chỉ được gán một lần cho mỗi Card. Frontend hiển thị avatar của các thành viên được gán trực tiếp trên Card trong Kanban view và hỗ trợ gán/hủy gán từ modal chi tiết Card.

\textbf{UC-013 — Thêm thành viên vào Board (100\%)}

Backend triển khai \texttt{POST /api/v1/boards-members} để thêm thành viên vào Board và \texttt{GET /api/v1/board-members?boardId=\&search=} để tìm kiếm thành viên theo tên trong Board. Frontend cung cấp giao diện quản lý thành viên Board với khả năng tìm kiếm và xác định vai trò (ADMIN, MEMBER, VIEWER) cho từng thành viên.

\subsection{Kết quả đạt được}

\subsubsection{Kết quả kiểm thử theo module}

Quá trình kiểm thử được thực hiện theo từng tuần phát triển. Phần lớn các test case tập trung vào luồng thành công (happy case) và một số trường hợp lỗi cơ bản; việc kiểm thử các trường hợp biên và luồng lỗi nâng cao chưa được thực hiện đầy đủ.

\textbf{Module Authentication (W3) — 14 test case}

\begin{table}[H]
\centering
\begin{tabular}{|p{2.5cm}|p{8cm}|p{2.5cm}|}
\hline
\textbf{Mã TC} & \textbf{Kịch bản} & \textbf{Kết quả} \\
\hline
TC01–TC03 & Đăng ký: hợp lệ, trùng email, thiếu trường bắt buộc & Pass \\
\hline
TC04–TC05 & Xác nhận OTP: mã đúng, mã sai & Pass \\
\hline
TC06–TC09 & Đăng nhập: hợp lệ, sai mật khẩu, chưa xác nhận email, kiểm tra cookie & Pass \\
\hline
TC10–TC12 & Làm mới token: hợp lệ, thiếu token, token hết hạn & Pass \\
\hline
TC13–TC14 & Gửi lại mã: user hợp lệ, user không tồn tại & Pass \\
\hline
\end{tabular}
\caption{Kết quả kiểm thử module Authentication}
\label{tab:test_auth}
\end{table}

\textbf{Module Workspace \& Board (W4) — 16 test case}

\begin{table}[H]
\centering
\begin{tabular}{|p{2.5cm}|p{8cm}|p{2.5cm}|}
\hline
\textbf{Mã TC} & \textbf{Kịch bản} & \textbf{Kết quả} \\
\hline
TC01–TC05 & Workspace: tạo mới, thiếu tên, token không hợp lệ, lấy danh sách, danh sách rỗng & Pass \\
\hline
TC06–TC08 & Board: tạo mới, Workspace không tồn tại, không có quyền & Pass \\
\hline
TC09–TC12 & Board: cập nhật, không tồn tại, xóa, xóa không có quyền & Pass \\
\hline
TC13–TC16 & List: tạo mới, Board không tồn tại, cập nhật, lấy danh sách & Pass \\
\hline
\end{tabular}
\caption{Kết quả kiểm thử module Workspace \& Board}
\label{tab:test_workspace_board}
\end{table}

\textbf{Module Card \& Checklist (W5) — 9 test case}

\begin{table}[H]
\centering
\begin{tabular}{|p{2.5cm}|p{8cm}|p{2.5cm}|}
\hline
\textbf{Mã TC} & \textbf{Kịch bản} & \textbf{Kết quả} \\
\hline
TC17–TC22 & Card: tạo mới, lấy chi tiết, cập nhật, di chuyển (drag-drop), xóa, lấy danh sách theo Board & Pass \\
\hline
TC23–TC25 & Checklist: tạo mới, tạo ChecklistItem, cập nhật trạng thái hoàn thành & Pass \\
\hline
\end{tabular}
\caption{Kết quả kiểm thử module Card \& Checklist}
\label{tab:test_card_checklist}
\end{table}

\textbf{Kiểm thử tích hợp Frontend (W6) — 9 test case}

\begin{table}[H]
\centering
\begin{tabular}{|p{2.5cm}|p{8cm}|p{2.5cm}|}
\hline
\textbf{Mã TC} & \textbf{Kịch bản} & \textbf{Kết quả} \\
\hline
TC01–TC03 & Luồng xác thực: đăng ký → xác nhận OTP → đăng nhập & Pass \\
\hline
TC04–TC06 & Xác thực lỗi: user đã tồn tại, sai mật khẩu, mã OTP sai & Pass \\
\hline
TC07–TC09 & Dashboard: tải giao diện, tạo Workspace, tạo Board & Pass \\
\hline
\end{tabular}
\caption{Kết quả kiểm thử tích hợp Frontend (W6)}
\label{tab:test_frontend_w6}
\end{table}

\textbf{Kiểm thử giao diện Board (W7) — 8 test case}

\begin{table}[H]
\centering
\begin{tabular}{|p{2.5cm}|p{8cm}|p{2.5cm}|}
\hline
\textbf{Mã TC} & \textbf{Kịch bản} & \textbf{Kết quả} \\
\hline
TC01–TC02 & Workspace Detail: tải giao diện, mời thành viên & Pass \\
\hline
TC04–TC08 & Board Kanban: tải Board, tạo List, tạo Card, xóa Card, xóa List & Pass \\
\hline
\end{tabular}
\caption{Kết quả kiểm thử giao diện Board (W7)}
\label{tab:test_board_ui}
\end{table}

\subsubsection{Các màn hình giao diện đã hoàn thành}

Hệ thống Frontend bao gồm 6 màn hình chính đã được phát triển và kiểm thử:

\begin{enumerate}
    \item \textbf{Màn hình Đăng ký (\texttt{/sign-up}):} Form nhập họ tên, email, mật khẩu với validation phía Client; thông báo lỗi khi tài khoản đã tồn tại; tự động chuyển hướng sang màn hình xác nhận OTP sau khi đăng ký.

% TODO-IMG: Chương 4 — Màn hình Đăng ký (/sign-up)
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{./images/chap4/screen-signup.png}
    \caption{Màn hình Đăng ký tài khoản}
    \label{fig:screen_signup}
\end{figure}

    \item \textbf{Màn hình Xác nhận OTP (\texttt{/confirm-sign-up}):} Ô nhập mã OTP 6 chữ số; nút gửi lại mã có bộ đếm thời gian; thông báo lỗi khi mã không hợp lệ; chuyển hướng về đăng nhập sau khi xác nhận.

% TODO-IMG: Chương 4 — Màn hình Xác nhận OTP (/confirm-sign-up)
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{./images/chap4/screen-confirm-otp.png}
    \caption{Màn hình Xác nhận OTP}
    \label{fig:screen_confirm_otp}
\end{figure}

    \item \textbf{Màn hình Đăng nhập (\texttt{/sign-in}):} Form email và mật khẩu; thông báo lỗi khi thông tin không đúng; lưu token vào HttpOnly cookie; chuyển hướng đến Dashboard.

% TODO-IMG: Chương 4 — Màn hình Đăng nhập (/sign-in)
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{./images/chap4/screen-signin.png}
    \caption{Màn hình Đăng nhập}
    \label{fig:screen_signin}
\end{figure}

    \item \textbf{Dashboard:} Hiển thị danh sách Workspace trong sidebar; hiển thị Board theo từng Workspace dưới dạng thumbnail với ảnh nền; xử lý trạng thái trống khi chưa có dữ liệu; tự động làm mới token khi phiên hết hạn.

% TODO-IMG: Chương 4 — Màn hình Dashboard
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{./images/chap4/screen-dashboard.png}
    \caption{Màn hình Dashboard}
    \label{fig:screen_dashboard}
\end{figure}

    \item \textbf{Màn hình Chi tiết Workspace (\texttt{/workspaces/:id}):} Thông tin Workspace, danh sách Board, danh sách thành viên với vai trò; modal mời thành viên bằng email.

% TODO-IMG: Chương 4 — Màn hình Chi tiết Workspace (/workspaces/:id)
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{./images/chap4/screen-workspace-detail.png}
    \caption{Màn hình Chi tiết Workspace}
    \label{fig:screen_workspace_detail}
\end{figure}

    \item \textbf{Màn hình Board Kanban (\texttt{/boards/:id}):} Giao diện Kanban với các List theo dạng cột ngang; hiển thị Card trong từng List; form inline tạo List và Card; modal chi tiết Card; tính năng kéo thả List và Card; quản lý thành viên Board; badge cảnh báo Due Date; avatar thành viên trên Card.

% TODO-IMG: Chương 4 — Màn hình Board Kanban (/boards/:id)
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{./images/chap4/screen-board-kanban.png}
    \caption{Màn hình Board Kanban}
    \label{fig:screen_board_kanban}
\end{figure}

\end{enumerate}

\subsection{Hình ảnh minh họa}

% TODO-IMG: Chương 4 — Minh họa tính năng kéo thả Card giữa các List
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{./images/chap4/feature-drag-drop.png}
    \caption{Tính năng kéo thả Card giữa các List}
    \label{fig:feature_drag_drop}
\end{figure}

% TODO-IMG: Chương 4 — Minh họa modal Chi tiết Card (Checklist, Due Date, Members)
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{./images/chap4/feature-card-detail.png}
    \caption{Modal Chi tiết Card với Checklist, Due Date và thành viên}
    \label{fig:feature_card_detail}
\end{figure}

\subsection{Đánh giá kết quả}

\subsubsection{Ưu điểm}

Qua quá trình phát triển và kiểm thử, hệ thống quản lý công việc Nello đạt được một số ưu điểm nổi bật:

\begin{itemize}
    \item \textbf{Kiến trúc phân tách rõ ràng:} Frontend (ReactJS) và Backend (Spring Boot) hoàn toàn độc lập, giao tiếp qua RESTful API chuẩn hóa, tạo điều kiện bảo trì và mở rộng từng thành phần một cách linh hoạt.
    \item \textbf{Bảo mật theo tiêu chuẩn:} Toàn bộ xác thực được ủy thác cho Amazon Cognito. Token lưu trong HttpOnly cookie giúp bảo vệ khỏi tấn công XSS; tài nguyên Backend và cơ sở dữ liệu được cô lập trong mạng nội bộ VPC.
    \item \textbf{Trải nghiệm người dùng trực quan:} Giao diện Kanban quen thuộc với hỗ trợ kéo thả và cơ chế optimistic update cho phép người dùng thấy phản hồi tức thì mà không cần chờ phản hồi từ Backend.
    \item \textbf{Codebase chất lượng:} Áp dụng DTO và validation, xử lý ngoại lệ tập trung (Exception Handler), định dạng response chuẩn hóa trên toàn bộ API.
    \item \textbf{Thiết kế cơ sở dữ liệu hợp lý:} Áp dụng cascade delete, ràng buộc UNIQUE, trường \texttt{position} kiểu TEXT linh hoạt phù hợp với thuật toán sắp xếp kéo thả.
\end{itemize}

\subsubsection{Hạn chế}

Bên cạnh các kết quả đạt được, hệ thống còn tồn tại một số hạn chế cần được khắc phục trong giai đoạn tiếp theo:

\begin{itemize}
    \item \textbf{UC-005 và UC-007 (Quản lý Workspace, Quản lý Board) chưa hoàn thiện (30\%):} Phần đọc dữ liệu đã hoạt động đầy đủ trên cả Backend và Frontend. Tuy nhiên, tính năng cập nhật và xóa Workspace chưa có endpoint Backend, còn tính năng cập nhật và xóa Board tuy đã có API Backend (\texttt{PUT} và \texttt{DELETE /api/v1/boards/:id}) nhưng chưa được tích hợp vào giao diện Frontend.
    \item \textbf{Kiểm thử chưa bao phủ đầy đủ các trường hợp biên và luồng lỗi:} Phần lớn các test case được thực hiện tập trung vào luồng thành công (happy case) và một số lỗi cơ bản. Các kịch bản lỗi nâng cao như dữ liệu vượt giới hạn, truy cập đồng thời, mất kết nối hoặc dữ liệu không nhất quán chưa được kiểm thử, dẫn đến nguy cơ tồn tại lỗi tiềm ẩn trong các điều kiện sử dụng thực tế.
    \item \textbf{UC-010 (Kéo thả Card và List) hoàn thành 80\%:} Thuật toán tính \texttt{position} dựa trên \texttt{beforeId}/\texttt{afterId} chưa xử lý đầy đủ các trường hợp biên khi kéo Card hoặc List vào vị trí đầu tiên (không có \texttt{beforeId}) hoặc cuối cùng (không có \texttt{afterId}), dẫn đến sai lệch thứ tự hiển thị sau một số thao tác kéo thả liên tiếp.
    \item \textbf{Phân quyền VIEWER chưa hoàn thiện:} Vai trò VIEWER được định nghĩa trong hệ thống nhưng chưa được áp dụng đầy đủ ở cả tầng Backend (kiểm tra quyền tại API endpoint) và tầng Frontend (ẩn/vô hiệu hóa các thao tác chỉnh sửa).
    \item \textbf{Chưa triển khai lên môi trường Staging:} Hệ thống hiện chỉ hoạt động trên môi trường phát triển cục bộ. Nguyên nhân chính là do xung đột cấu hình HttpOnly cookie khi Frontend và Backend được triển khai trên các domain khác nhau, đòi hỏi phải điều chỉnh chính sách \texttt{SameSite} và CORS.
    \item \textbf{Chưa có tính năng đồng bộ thời gian thực:} Hệ thống hoạt động theo mô hình request-response thuần túy. Khi một thành viên thực hiện thay đổi, các thành viên khác đang xem cùng Board cần tải lại trang thủ công để cập nhật dữ liệu, ảnh hưởng đến trải nghiệm cộng tác.
\end{itemize}

\clearpage

\section{CHƯƠNG 5: KẾT LUẬN VÀ HƯỚNG PHÁT TRIỂN}

\subsection{Kết luận}

\subsubsection{Các công việc đã thực hiện}

Trong khuôn khổ học phần Project I, toàn bộ quá trình nghiên cứu, thiết kế và phát triển hệ thống quản lý công việc Nello đã được thực hiện trong 8 tuần liên tục, từ ngày 30/03/2026 đến ngày 24/05/2026. Các công việc chính đã hoàn thành bao gồm:

\begin{itemize}
    \item \textbf{Phân tích và thiết kế hệ thống:} Xác định yêu cầu chức năng và phi chức năng; thiết kế kiến trúc tổng thể theo mô hình client-server; thiết kế cơ sở dữ liệu quan hệ gồm 12 bảng với các ràng buộc toàn vẹn dữ liệu đầy đủ; thiết kế luồng xác thực dựa trên Amazon Cognito và JWT.
    \item \textbf{Phát triển Backend:} Xây dựng hoàn chỉnh 14 nhóm RESTful API endpoint bằng Java Spring Boot, bao phủ toàn bộ 7 module nghiệp vụ: Xác thực, Người dùng, Workspace, Board, List, Card, Checklist và Card Member. Áp dụng mô hình phân lớp \texttt{controller} -- \texttt{service} -- \texttt{repository}, xử lý ngoại lệ tập trung, chuẩn hóa định dạng response và tích hợp Spring Data JPA với PostgreSQL.
    \item \textbf{Phát triển Frontend:} Xây dựng 6 màn hình giao diện bằng ReactJS bao gồm luồng xác thực đầy đủ (đăng ký, xác nhận OTP, đăng nhập), Dashboard quản lý Workspace và Board, màn hình chi tiết Workspace và màn hình Kanban Board. Triển khai thành công tính năng kéo thả với cơ chế optimistic update và quản lý trạng thái phức tạp.
    \item \textbf{Kiểm thử:} Thực hiện hơn 30 test case theo từng tuần phát triển, bao phủ cả luồng thành công (happy case) và các trường hợp lỗi cơ bản trên tất cả các module.
    \item \textbf{Thiết kế hạ tầng đám mây:} Thiết kế và lập kế hoạch triển khai kiến trúc AWS hoàn chỉnh bao gồm VPC, EC2, RDS, S3, CloudFront, ALB và Amazon Cognito theo mô hình bảo mật nhiều lớp.
\end{itemize}

\subsubsection{Các mục tiêu đã đạt được}

Đối chiếu với các mục tiêu đề ra ban đầu, các kết quả đạt được như sau:

\begin{itemize}
    \item \textbf{Xây dựng nền tảng quản lý khoa học:} Hoàn thành. Hệ thống hỗ trợ đầy đủ việc tổ chức công việc theo cấu trúc phân cấp Workspace $\rightarrow$ Board $\rightarrow$ List $\rightarrow$ Card, đáp ứng nhu cầu quản lý dự án đa cấp.
    \item \textbf{Quản lý thẻ công việc chi tiết:} Hoàn thành. Card hỗ trợ đầy đủ các thuộc tính gồm tiêu đề, mô tả, ngày bắt đầu, ngày đến hạn, gán thành viên và danh sách kiểm tra (Checklist) với thanh tiến độ trực quan.
    \item \textbf{Tích hợp cơ chế tương tác trực quan:} Hoàn thành. Tính năng kéo thả hoạt động ổn định cho cả List và Card trong Board, với phản hồi giao diện tức thì thông qua cơ chế optimistic update.
    \item \textbf{Đồng bộ dữ liệu và cộng tác:} Hoàn thành một phần. Các cập nhật về trạng thái công việc, phân công nhân sự và thời hạn được đồng bộ hóa với Backend sau mỗi thao tác; tuy nhiên, đồng bộ thời gian thực giữa các thành viên đang xem cùng một Board chưa được triển khai.
\end{itemize}

\subsubsection{Các kết quả đạt được}

Sau 8 tuần triển khai, hệ thống Nello đã đạt được những kết quả cụ thể sau:

\begin{table}[H]
\centering
\begin{tabular}{|p{6cm}|p{8cm}|}
\hline
\textbf{Hạng mục} & \textbf{Kết quả} \\
\hline
Số API endpoint hoàn thành & 14 nhóm (toàn bộ Backend) \\
\hline
Số màn hình Frontend & 6 màn hình chính \\
\hline
Số test case đã kiểm thử & Hơn 30 test case \\
\hline
Tỷ lệ test case pass & 90\% \\
\hline
Trạng thái Backend & Hoàn chỉnh \\
\hline
Trạng thái Frontend & Hoàn chỉnh các chức năng cốt lõi \\
\hline
Môi trường triển khai & Local development \\
\hline
\end{tabular}
\caption{Tổng hợp kết quả học phần Project I}
\label{tab:final_summary}
\end{table}

Hệ thống đã chứng minh tính khả thi của việc xây dựng một ứng dụng quản lý công việc theo mô hình Kanban với kiến trúc phân tách Frontend/Backend, tích hợp dịch vụ đám mây AWS và đảm bảo các tiêu chuẩn bảo mật cơ bản.

\subsection{Các vấn đề còn tồn tại}

\subsubsection{Chức năng chưa hoàn thiện}

Dựa trên kết quả đánh giá theo Use Case ở Chương 4, hai Use Case chưa đạt mức hoàn thành toàn bộ:

\begin{itemize}
    \item \textbf{UC-005 — Quản lý Workspace (30\%):} Phần đọc dữ liệu (xem danh sách, xem chi tiết Workspace) và tạo Workspace đã hoàn thành. Tuy nhiên, endpoint cập nhật thông tin Workspace và endpoint xóa Workspace chưa được xây dựng ở tầng Backend, đồng thời giao diện Frontend tương ứng cho hai thao tác này cũng chưa được triển khai.
    \item \textbf{UC-007 — Quản lý Board (30\%):} Phần hiển thị Board theo mô hình Kanban và tạo Board đã hoàn thành. API cập nhật (\texttt{PUT /api/v1/boards/:id}) và xóa (\texttt{DELETE /api/v1/boards/:id}) đã có ở tầng Backend nhưng chưa được tích hợp vào giao diện Frontend.
    \item \textbf{UC-010 — Kéo thả Card và List (80\%):} Chức năng kéo thả hoạt động với hầu hết các trường hợp thông thường. Phần còn thiếu là xử lý các trường hợp biên khi kéo Card hoặc List vào vị trí đầu tiên hoặc cuối cùng của danh sách, dẫn đến sai lệch thứ tự hiển thị.
\end{itemize}

Ngoài ra, một số tính năng đã được xác định là cần thiết nhưng nằm ngoài phạm vi thực hiện của học phần:

\begin{table}[H]
\centering
\begin{tabular}{|p{4cm}|p{10cm}|}
\hline
\textbf{Tính năng} & \textbf{Mô tả} \\
\hline
Quản lý nhãn dán (Tag) & Tạo, gắn, tìm kiếm và lọc Card theo Tag màu sắc. \\
\hline
Hệ thống thông báo & Thông báo khi Card được giao, khi gần đến hạn hoặc khi được mời vào Workspace/Board. \\
\hline
Nhật ký hoạt động & Ghi nhận lịch sử tạo, cập nhật, xóa dữ liệu; hiển thị timeline hoạt động trong Card. \\
\hline
Xem lịch (Calendar View) & Hiển thị Card theo due date trên giao diện lịch tháng. \\
\hline
Upload tệp đính kèm & Đính kèm tệp tin vào Card thông qua Amazon S3. \\
\hline
Đồng bộ thời gian thực & Cập nhật tự động giao diện khi thành viên khác thay đổi dữ liệu trong cùng Board. \\
\hline
\end{tabular}
\caption{Các tính năng chưa được triển khai}
\label{tab:unimplemented_features}
\end{table}

\subsubsection{Hạn chế kỹ thuật}

\begin{itemize}
    \item \textbf{Lỗi kéo thả ở vị trí đầu/cuối danh sách:} Thuật toán tính \texttt{position} dựa trên \texttt{beforeId}/\texttt{afterId} chưa xử lý đầy đủ các trường hợp biên khi kéo Card hoặc List về vị trí đầu tiên (không tồn tại \texttt{beforeId}) hoặc vị trí cuối cùng (không tồn tại \texttt{afterId}), dẫn đến sai lệch thứ tự hiển thị sau một số thao tác kéo thả liên tiếp.
    \item \textbf{Phân quyền VIEWER chưa hoàn thiện:} Vai trò VIEWER được định nghĩa trong cơ sở dữ liệu nhưng chưa được kiểm soát đầy đủ ở tầng Backend (thiếu middleware kiểm tra quyền trên các write operation) và tầng Frontend (chưa ẩn hoặc vô hiệu hóa các nút thao tác chỉnh sửa tương ứng).
    \item \textbf{Chưa triển khai lên môi trường Staging:} Hệ thống hiện chỉ vận hành trên môi trường phát triển cục bộ. Vấn đề kỹ thuật cốt lõi là xung đột cấu hình HttpOnly cookie khi Frontend và Backend được đặt trên các domain khác nhau, đòi hỏi phải điều chỉnh chính sách \texttt{SameSite} và cấu hình CORS phù hợp với môi trường production.
    \item \textbf{Chưa có đồng bộ thời gian thực:} Hệ thống hoạt động theo mô hình request-response thuần túy. Khi một thành viên thực hiện thay đổi, các thành viên khác đang xem cùng Board cần tải lại trang thủ công để nhận cập nhật mới nhất.
    \item \textbf{Kiểm thử chưa bao phủ đầy đủ:} Phần lớn test case tập trung vào luồng thành công và một số lỗi cơ bản. Các kịch bản nâng cao như dữ liệu vượt giới hạn, truy cập đồng thời, mất kết nối mạng và dữ liệu không nhất quán chưa được kiểm thử, tiềm ẩn nguy cơ lỗi trong môi trường thực tế.
\end{itemize}

\subsection{Phương hướng phát triển}

\subsubsection{Ưu tiên cao — Hoàn thiện hệ thống hiện tại}

\begin{itemize}
    \item \textbf{Sửa lỗi thuật toán kéo thả:} Rà soát và hoàn thiện thuật toán tính \texttt{beforeId}/\texttt{afterId}, đảm bảo xử lý đúng tất cả các trường hợp biên bao gồm kéo vào đầu danh sách, cuối danh sách và danh sách trống; đồng thời đảm bảo tính nhất quán giữa trạng thái Frontend và dữ liệu Backend.
    \item \textbf{Hoàn thiện phân quyền VIEWER:} Bổ sung kiểm tra vai trò tại các API endpoint write operation ở tầng Backend và ẩn hoặc vô hiệu hóa các thành phần giao diện thao tác tương ứng khi người dùng có vai trò VIEWER.
    \item \textbf{Giải quyết vấn đề triển khai Staging:} Lựa chọn và triển khai chiến lược quản lý token phù hợp với môi trường cross-domain, bao gồm cấu hình \texttt{SameSite=None; Secure} cho cookie hoặc chuyển sang cơ chế lưu token trong \texttt{Authorization} header, sau đó tiến hành triển khai hệ thống lên môi trường Staging trên hạ tầng AWS đã thiết kế.
    \item \textbf{Hoàn thiện UC-005 và UC-007 (Quản lý Workspace, Quản lý Board):} Xây dựng endpoint cập nhật và xóa Workspace ở tầng Backend; tích hợp endpoint cập nhật/xóa Board vào giao diện Frontend; bổ sung các màn hình tương ứng để người dùng có thể chỉnh sửa thông tin và xóa Workspace, Board trực tiếp từ giao diện web.
\end{itemize}

\subsubsection{Ưu tiên trung bình — Mở rộng tính năng}

\begin{itemize}
    \item \textbf{Đồng bộ thời gian thực (Realtime Websocket):} Tích hợp Spring WebSocket với giao thức STOMP vào Backend và SockJS/STOMP client vào Frontend để đẩy sự kiện cập nhật dữ liệu (tạo, cập nhật, xóa Card; thay đổi Checklist; kéo thả) đến toàn bộ thành viên đang hoạt động trên cùng Board, nâng cao đáng kể trải nghiệm cộng tác nhóm.
    \item \textbf{Quản lý nhãn dán (Tag):} Triển khai tính năng tạo, chỉnh sửa và xóa Tag với màu sắc tùy chọn; gắn nhiều Tag cho một Card; tìm kiếm và lọc Card theo Tag trong Board.
    \item \textbf{Hệ thống thông báo (Notification):} Xây dựng cơ chế thông báo trong ứng dụng khi Card được giao cho người dùng, khi công việc sắp đến hạn hoặc đã quá hạn, và khi được mời vào Workspace hoặc Board.
\end{itemize}

\subsubsection{Ưu tiên thấp — Phát triển dài hạn}

\begin{itemize}
    \item \textbf{Nhật ký hoạt động (Activity Log):} Ghi nhận lịch sử toàn bộ thao tác tạo, cập nhật và xóa dữ liệu; hiển thị timeline hoạt động theo thời gian thực trong modal Card Detail và màn hình Board.
    \item \textbf{Các chế độ xem thay thế:} Phát triển Calendar View hiển thị Card theo due date trên lịch tháng và Timeline View trình bày tiến trình công việc theo dòng thời gian, giúp người dùng có thêm góc nhìn tổng quan về tiến độ dự án.
    \item \textbf{Upload tệp đính kèm:} Tích hợp khả năng đính kèm tệp tin vào Card thông qua Amazon S3, hỗ trợ xem trước và tải xuống trực tiếp từ giao diện Card Detail.
    \item \textbf{Tối ưu hóa hiệu năng:} Khi mở rộng quy mô, bổ sung cơ chế phân trang (pagination) cho các danh sách lớn, áp dụng caching với Redis cho các truy vấn thường xuyên và tối ưu hóa query PostgreSQL với index phù hợp.
\end{itemize}