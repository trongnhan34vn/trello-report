...
Mô hình dữ liệu nghiệp vụ được xác định xoay quanh bảy thực thể chính theo cấu trúc phân cấp:
\begin{itemize}
    \item \textbf{User}: người dùng đã đăng ký tài khoản trong hệ thống, là chủ thể thực hiện mọi thao tác.
...
\paragraph{Mô hình phân lớp Controller -- Service -- Repository} \mbox{}\\

Tầng Backend của hệ thống được tổ chức theo mô hình phân lớp ba tầng: \textbf{Controller Layer} tiếp nhận HTTP request, xác thực JWT và ủy quyền xử lý xuống tầng dưới; \textbf{Service Layer} chứa toàn bộ logic nghiệp vụ, điều phối giữa các repository và dịch vụ ngoài (Amazon Cognito, S3); \textbf{Repository Layer} chịu trách nhiệm truy xuất và lưu trữ dữ liệu thông qua Spring Data JPA, giao tiếp trực tiếp với PostgreSQL. Mô hình này đảm bảo tách biệt rõ trách nhiệm giữa các tầng, giúp dễ dàng kiểm thử và bảo trì từng thành phần độc lập.
...
\begin{itemize}
    \item \textbf{Giới thiệu:} Amazon Cognito là dịch vụ quản lý danh tính và xác thực người dùng của AWS, hỗ trợ các luồng xác thực theo chuẩn OAuth 2.0 và OpenID Connect.
    \item \textbf{Vai trò trong hệ thống:} Được tích hợp với Backend qua AWS SDK for Java để xử lý toàn bộ luồng xác thực. Client không gọi trực tiếp Cognito mà thông qua Backend; Backend giao tiếp với Cognito để đăng ký, đăng nhập và nhận JWT (access token, refresh token) rồi trả về cho Client.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Vai trò trong hệ thống:} Được dự kiến sử dụng để hosting cơ sở dữ liệu PostgreSQL trên môi trường đám mây, thay thế cơ sở dữ liệu cục bộ trong quá trình phát triển, nhằm chuẩn bị cho việc triển khai lên môi trường Staging trong giai đoạn tiếp theo.
\end{itemize}

\paragraph{Amazon EC2 (Elastic Compute Cloud)}
\begin{itemize}
    \item \textbf{Giới thiệu:} Amazon EC2 là dịch vụ máy chủ ảo của AWS, cung cấp khả năng điện toán linh hoạt trên đám mây.
    \item \textbf{Vai trò trong hệ thống:} Được sử dụng làm máy chủ đặt trong Private Subnet để triển khai Backend (Spring Boot) và Frontend (ReactJS qua Nginx và Docker). Việc đặt trong mạng private giúp cách ly hoàn toàn khỏi Internet, tăng cường bảo mật.
\end{itemize}

\paragraph{Amazon ALB (Application Load Balancer)}
\begin{itemize}
    \item \textbf{Giới thiệu:} Amazon ALB là dịch vụ cân bằng tải tầng ứng dụng (Layer 7) của AWS, phân phối lưu lượng HTTP/HTTPS đến các máy chủ đích.
    \item \textbf{Vai trò trong hệ thống:} Đóng vai trò điểm truy cập (entry point) duy nhất cho hệ thống, tiếp nhận request từ người dùng và phân phối đến các EC2 instance, đảm bảo tính sẵn sàng và khả năng mở rộng.
\end{itemize}

\paragraph{Amazon S3 (Simple Storage Service)}
\begin{itemize}
    \item \textbf{Giới thiệu:} Amazon S3 là dịch vụ lưu trữ đối tượng (object storage) của AWS với độ bền và tính bảo mật cao.
    \item \textbf{Vai trò trong hệ thống:} Được sử dụng chuyên biệt để lưu trữ tài nguyên đa phương tiện do người dùng tải lên như ảnh đại diện (avatar) và tệp đính kèm trong Card.
\end{itemize}

\paragraph{Amazon CloudFront}
\begin{itemize}
    \item \textbf{Giới thiệu:} Amazon CloudFront là dịch vụ mạng phân phối nội dung (CDN) của AWS, giảm độ trễ truyền tải thông qua hệ thống edge locations toàn cầu.
    \item \textbf{Vai trò trong hệ thống:} Đóng vai trò trung gian phân phối tài nguyên đa phương tiện từ S3 đến người dùng, cung cấp HTTPS bảo mật và hạn chế truy cập công khai trực tiếp vào S3.
\end{itemize}

\paragraph{JWT (JSON Web Token)}
\begin{itemize}
    \item \textbf{Giới thiệu:} JWT là tiêu chuẩn mở (RFC 7519) định nghĩa cách truyền tải thông tin an toàn giữa các bên dưới dạng JSON được ký số, gồm ba phần: Header, Payload và Signature.
    \item \textbf{Vai trò trong hệ thống:} Được Cognito phát hành sau đăng nhập thành công và lưu trong HttpOnly cookie phía Client. Backend xác thực chữ ký JWT trên mỗi request API để kiểm tra danh tính người dùng mà không cần truy vấn cơ sở dữ liệu.
\end{itemize}
...
\paragraph{Docker}
\begin{itemize}
    \item \textbf{Giới thiệu:} Docker là nền tảng container hóa mã nguồn mở, đóng gói ứng dụng cùng toàn bộ phụ thuộc vào một container có thể chạy nhất quán trên mọi môi trường.
\begin{figure}[H]
    \centering
...
\end{figure}
    \item \textbf{Vai trò trong hệ thống:} Ứng dụng Frontend (ReactJS) được đóng gói thành Docker image và triển khai trên EC2, đảm bảo môi trường chạy nhất quán giữa môi trường phát triển và AWS.
\end{itemize}

\paragraph{Nginx}
\begin{itemize}
    \item \textbf{Giới thiệu:} Nginx là web server hiệu năng cao và reverse proxy mã nguồn mở, xử lý đồng thời lượng lớn kết nối với mức tiêu thụ tài nguyên thấp.
\begin{figure}[H]
    \centering
...
\end{figure}
    \item \textbf{Vai trò trong hệ thống:} Được cài đặt trong Docker container trên EC2 Frontend, phục vụ các tệp tĩnh của ReactJS và xử lý client-side routing bằng cách chuyển hướng mọi request về \texttt{index.html}.
\end{itemize}
...
\hline
FR-33 & Cập nhật hồ sơ cá nhân & Người dùng chỉnh sửa thông tin cá nhân như họ tên, ảnh đại diện, số điện thoại và địa chỉ. \\
\hline
\end{tabular}
...
\hline
FR-08 & Cập nhật Workspace & Người dùng có vai trò ADMIN chỉnh sửa tên và mô tả của Workspace. \\
\hline
FR-09 & Quản lý thành viên Workspace & Người dùng có vai trò ADMIN hoặc MEMBER thêm thành viên vào Workspace bằng địa chỉ email. \\
\hline
FR-30 & Xóa Workspace & Người dùng (ADMIN) xóa vĩnh viễn Workspace cùng toàn bộ Board, List, Card, Checklist và ChecklistItem bên trong. \\
\hline
\end{tabular}
...
\textbf{Mã} & \textbf{Tên chức năng} & \textbf{Mô tả} \\
\hline
FR-10 & Tạo Board & Người dùng có vai trò ADMIN hoặc MEMBER trong Workspace tạo bảng công việc mới với tiêu đề và hình nền tùy chọn. \\
\hline
FR-11 & Xem Board & Tải và hiển thị toàn bộ danh sách List và Card trong Board theo mô hình Kanban. \\
\hline
FR-12 & Cập nhật Board & Người dùng có vai trò ADMIN chỉnh sửa tiêu đề của Board. \\
\hline
FR-13 & Quản lý thành viên Board & Người dùng có vai trò ADMIN hoặc MEMBER thêm bất kỳ người dùng đã đăng ký vào Board với vai trò mặc định MEMBER. \\
\hline
FR-31 & Xóa Board & Người dùng có quyền ADMIN xóa vĩnh viễn Board cùng toàn bộ List, Card, Checklist và ChecklistItem bên trong. \\
\hline
\end{tabular}
...
\textbf{Mã} & \textbf{Tên chức năng} & \textbf{Mô tả} \\
\hline
FR-14 & Tạo List & Người dùng có vai trò ADMIN hoặc MEMBER tạo cột danh sách công việc mới trong Board. \\
\hline
FR-15 & Cập nhật List & Người dùng có vai trò ADMIN hoặc MEMBER đổi tên hoặc thay đổi vị trí của List trong Board. \\
\hline
FR-16 & Xóa List & Người dùng có vai trò ADMIN hoặc MEMBER xóa List và toàn bộ Card, Checklist, ChecklistItem bên trong theo cơ chế cascade. \\
\hline
\end{tabular}
...
\textbf{Mã} & \textbf{Tên chức năng} & \textbf{Mô tả} \\
\hline
FR-17 & Tạo Card & Người dùng có vai trò ADMIN hoặc MEMBER tạo thẻ công việc mới trong một List cụ thể với tiêu đề ban đầu. \\
\hline
...
\hline
FR-19 & Cập nhật Card & Người dùng có vai trò ADMIN hoặc MEMBER chỉnh sửa tiêu đề, mô tả, ngày bắt đầu, ngày đến hạn và trạng thái hoàn thành của Card. \\
\hline
FR-20 & Xóa Card & Người dùng có vai trò ADMIN hoặc MEMBER xóa thẻ công việc cùng toàn bộ Checklist, ChecklistItem và CardMember liên quan theo cơ chế cascade. \\
\hline
FR-22 & Gán thành viên vào Card & Người dùng có vai trò ADMIN hoặc MEMBER gán hoặc hủy gán thành viên chịu trách nhiệm thực hiện Card. Avatar thành viên được hiển thị trực tiếp trên Card. \\
\hline
\end{tabular}
...
\textbf{Mã} & \textbf{Tên chức năng} & \textbf{Mô tả} \\
\hline
FR-23 & Kéo thả Card & Người dùng có vai trò ADMIN hoặc MEMBER di chuyển Card sang List khác hoặc sắp xếp lại thứ tự Card trong cùng một List; cập nhật \texttt{position} (và \texttt{listId} nếu đổi List) đồng bộ với Backend. \\
\hline
FR-24 & Kéo thả List trong Board & Người dùng có vai trò ADMIN hoặc MEMBER thay đổi thứ tự hiển thị của các List trong Board. \\
\hline
\end{tabular}
...
\textbf{Mã} & \textbf{Tên chức năng} & \textbf{Mô tả} \\
\hline
FR-26 & Tạo Checklist & Người dùng có vai trò ADMIN hoặc MEMBER tạo danh sách kiểm tra mới trong Card. \\
\hline
FR-27 & Cập nhật Checklist & Người dùng có vai trò ADMIN hoặc MEMBER đổi tên Checklist. \\
\hline
FR-28 & Xóa Checklist & Người dùng có vai trò ADMIN hoặc MEMBER xóa Checklist và toàn bộ ChecklistItem liên quan. \\
\hline
FR-29 & Quản lý ChecklistItem & Người dùng có vai trò ADMIN hoặc MEMBER thêm hạng mục với tên và ngày đến hạn (tùy chọn), cập nhật trạng thái hoàn thành và xóa các hạng mục công việc con. Hiển thị thanh tiến độ (progress bar) theo tỷ lệ phần trăm hoàn thành. \\
\hline
\end{tabular}
...
\textbf{UC-01: Quản lý xác thực} -- Bao gồm đăng ký tài khoản, xác nhận OTP, gửi lại OTP, đăng nhập, làm mới access token và đăng xuất.

\textbf{UC-02: Quản lý người dùng} -- Bao gồm xem thông tin cá nhân và chỉnh sửa thông tin cá nhân.

\textbf{UC-03: Quản lý Workspace} -- Bao gồm tạo, xem danh sách, xem chi tiết Workspace, mời thành viên, xem danh sách thành viên, cập nhật thông tin và xóa Workspace.
...
\hline
UC-03.6 & Cập nhật thông tin Workspace \\
\hline
UC-03.7 & Xóa Workspace \\
\hline
UC-04.1 & Tạo Board \\
\hline
...
\paragraph{Phân quyền theo vai trò} \mbox{}\\

Hệ thống định nghĩa ba vai trò: \textbf{ADMIN} (quản trị viên), \textbf{MEMBER} (thành viên) và \textbf{VIEWER} (người xem). Vai trò được gán \textbf{riêng biệt cho từng Workspace và từng Board} — một người dùng có thể là ADMIN trong Workspace này nhưng chỉ là MEMBER hoặc VIEWER trong Workspace khác, và tương tự ở cấp Board.

Nguyên tắc phân quyền như sau: \textbf{VIEWER} chỉ có quyền xem; \textbf{ADMIN} có toàn quyền, bao gồm quyền chỉnh sửa thông tin và xóa Workspace hoặc Board; \textbf{MEMBER} thừa hưởng tất cả các quyền còn lại của ADMIN, ngoại trừ các thao tác tác động trực tiếp đến thông tin hoặc sự tồn tại của Workspace và Board (cập nhật thông tin, xóa). Quyền hạn được kiểm tra tại tầng Backend (API endpoint) lẫn tầng Frontend (ẩn hoặc vô hiệu hóa thao tác tương ứng).

\begin{table}[H]
\centering
\begin{tabular}{|p{7cm}|p{2cm}|p{2cm}|p{2cm}|}
\hline
\textbf{Thao tác} & \textbf{ADMIN} & \textbf{MEMBER} & \textbf{VIEWER} \\
\hline
Cập nhật thông tin Workspace & $\surd$ & -- & -- \\
\hline
Xóa Workspace & $\surd$ & -- & -- \\
\hline
Mời thành viên vào Workspace & $\surd$ & $\surd$ & -- \\
\hline
Tạo Board & $\surd$ & $\surd$ & -- \\
\hline
Cập nhật thông tin Board & $\surd$ & -- & -- \\
\hline
Xóa Board & $\surd$ & -- & -- \\
\hline
Thêm thành viên vào Board & $\surd$ & $\surd$ & -- \\
\hline
Tạo / cập nhật / xóa List & $\surd$ & $\surd$ & -- \\
\hline
Tạo / cập nhật / xóa Card & $\surd$ & $\surd$ & -- \\
\hline
Thêm / xóa Checklist và ChecklistItem & $\surd$ & $\surd$ & -- \\
\hline
Gán / hủy gán thành viên vào Card & $\surd$ & $\surd$ & -- \\
\hline
Xem Board, List, Card & $\surd$ & $\surd$ & $\surd$ \\
\hline
\end{tabular}
\caption{Phân quyền theo vai trò trong hệ thống}
\label{tab:role_permissions}
\end{table}
...
\hline
Luồng chính & 1. Hệ thống phát hiện access token hết hạn khi xử lý yêu cầu.\newline 2. Hệ thống sử dụng refresh token để yêu cầu Amazon Cognito cấp access token mới.\newline 3. Hệ thống cập nhật HttpOnly cookie với access token mới.\newline 4. Yêu cầu gốc của người dùng được thực hiện lại. \\
\hline
...
Tác nhân chính & Người dùng \\
\hline
Mô tả tóm tắt & Người dùng kết thúc phiên làm việc, hệ thống xóa cookie xác thực khỏi trình duyệt. \\
\hline
Điều kiện & Người dùng đã đăng nhập; access token và refresh token đang lưu trong HttpOnly cookie. \\
\hline
Luồng chính & 1. Người dùng nhấn Đăng xuất.\newline 2. Hệ thống gọi API đăng xuất, xóa toàn bộ HttpOnly cookie (access token và refresh token) khỏi trình duyệt.\newline 3. Hệ thống chuyển hướng người dùng về trang đăng nhập. \\
\hline
Luồng thay thế & Lỗi không xác định: Hệ thống vẫn xóa cookie phía Client và chuyển hướng về trang đăng nhập để đảm bảo phiên làm việc được kết thúc. \\
\hline
\end{tabular}
...
\hline
Luồng chính & 1. Người dùng truy cập trang thông tin cá nhân.\newline 2. Hệ thống xác thực phiên làm việc và truy vấn thông tin hồ sơ người dùng.\newline 3. Hệ thống hiển thị họ tên, email, ảnh đại diện và các thông tin cá nhân khác. \\
\hline
...
Điều kiện & Người dùng đã đăng nhập thành công. \\
\hline
Luồng chính & 1. Người dùng truy cập trang Dashboard.\newline 2. Hệ thống truy vấn danh sách Workspace mà người dùng sở hữu hoặc là thành viên.\newline 3. Hệ thống hiển thị danh sách Workspace kèm tên và mô tả. \\
\hline
...
\hline
Luồng chính & 1. Người dùng chọn một Workspace từ danh sách.\newline 2. Hệ thống kiểm tra quyền truy cập và tải thông tin chi tiết Workspace.\newline 3. Hệ thống hiển thị tên, mô tả, loại Workspace và danh sách Board bên trong. \\
\hline
...
Tác nhân chính & Người dùng \\
\hline
Mô tả tóm tắt & Người dùng có vai trò ADMIN hoặc MEMBER mời người dùng khác tham gia vào Workspace. \\
\hline
Điều kiện & Người dùng đã đăng nhập và có vai trò ADMIN hoặc MEMBER trong Workspace. \\
\hline
Luồng chính & 1. Người dùng truy cập trang quản lý thành viên Workspace.\newline 2. Người dùng tìm kiếm thành viên cần mời theo email.\newline 3. Người dùng chọn thành viên từ kết quả tìm kiếm và nhấn Mời.\newline 4. Hệ thống xác minh người dùng tồn tại và chưa là thành viên của Workspace.\newline 5. Hệ thống thêm người dùng vào Workspace với vai trò MEMBER và cập nhật danh sách thành viên. \\
\hline
Luồng thay thế & Người dùng được mời không tồn tại trong hệ thống: Hệ thống hiển thị thông báo lỗi.\newline Người dùng đã là thành viên của Workspace: Hệ thống hiển thị thông báo trùng lặp.\newline Người dùng có vai trò VIEWER: Hệ thống trả về lỗi HTTP 403. \\
\hline
\end{tabular}
...
Điều kiện & Người dùng đã đăng nhập và là thành viên của Workspace. \\
\hline
Luồng chính & 1. Người dùng truy cập trang thành viên của Workspace.\newline 2. Hệ thống truy vấn danh sách thành viên cùng vai trò trong Workspace.\newline 3. Hệ thống hiển thị danh sách thành viên kèm tên, email và vai trò. \\
\hline
Luồng thay thế & Workspace không tồn tại: Hệ thống trả về lỗi HTTP 404.\newline Người dùng không có quyền truy cập: Hệ thống trả về lỗi HTTP 403. \\
\hline
\end{tabular}
...
Mã Use Case & UC-03.6 \\
\hline
Tên Use Case & Cập nhật thông tin Workspace \\
\hline
Tác nhân chính & Người dùng \\
\hline
Mô tả tóm tắt & Người dùng có vai trò ADMIN chỉnh sửa tên và mô tả của Workspace. \\
\hline
Điều kiện & Người dùng đã đăng nhập và có vai trò ADMIN trong Workspace. \\
\hline
Luồng chính & 1. Người dùng truy cập trang cài đặt Workspace.\newline 2. Hệ thống hiển thị form chỉnh sửa tên và mô tả hiện tại.\newline 3. Người dùng thay đổi thông tin và nhấn Lưu.\newline 4. Hệ thống xác thực dữ liệu và cập nhật thông tin Workspace.\newline 5. Hệ thống hiển thị thông báo cập nhật thành công. \\
\hline
Luồng thay thế & Tên Workspace bị bỏ trống: Hệ thống hiển thị thông báo lỗi và không cho phép lưu.\newline Người dùng không có quyền ADMIN: Hệ thống trả về lỗi HTTP 403. \\
\hline
\end{tabular}
...
Tác nhân chính & Người dùng \\
\hline
Mô tả tóm tắt & Người dùng có vai trò ADMIN xóa vĩnh viễn Workspace cùng toàn bộ Board, List, Card, Checklist và ChecklistItem bên trong. \\
\hline
Điều kiện & Người dùng đã đăng nhập và có vai trò ADMIN trong Workspace. \\
\hline
Luồng chính & 1. Người dùng truy cập trang cài đặt Workspace và chọn xóa.\newline 2. Hệ thống hiển thị hộp thoại xác nhận với cảnh báo về dữ liệu bị xóa vĩnh viễn.\newline 3. Người dùng xác nhận thao tác.\newline 4. Hệ thống xóa Workspace và toàn bộ Board, List, Card, Checklist, ChecklistItem bên trong theo cơ chế cascade.\newline 5. Hệ thống chuyển hướng người dùng về trang Dashboard. \\
\hline
Luồng thay thế & Người dùng không có quyền ADMIN: Hệ thống trả về lỗi HTTP 403.\newline Workspace không tồn tại: Hệ thống trả về lỗi HTTP 404. \\
\hline
\end{tabular}
...
Điều kiện & Người dùng đã đăng nhập và có vai trò ADMIN hoặc MEMBER trong Workspace. \\
\hline
Luồng chính & 1. Người dùng chọn chức năng tạo Board trong Workspace.\newline 2. Hệ thống hiển thị form nhập tên và chọn hình nền Board.\newline 3. Người dùng điền tên, chọn hình nền và nhấn Tạo.\newline 4. Hệ thống kiểm tra vai trò của người dùng trong Workspace (VIEWER bị từ chối).\newline 5. Hệ thống tạo Board và hiển thị Board mới trong danh sách Workspace. \\
\hline
Luồng thay thế & Người dùng có vai trò VIEWER trong Workspace: Hệ thống trả về lỗi HTTP 403.\newline Tên Board bị bỏ trống: Hệ thống trả về lỗi HTTP 400. \\
\hline
\end{tabular}
...
\hline
Luồng chính & 1. Người dùng chọn Board từ danh sách.\newline 2. Hệ thống kiểm tra quyền truy cập của người dùng trong Board.\newline 3. Hệ thống tải và hiển thị toàn bộ List và Card theo mô hình Kanban. \\
\hline
...
Tác nhân chính & Người dùng \\
\hline
Mô tả tóm tắt & Người dùng xóa vĩnh viễn Board cùng toàn bộ List, Card, Checklist và ChecklistItem bên trong. \\
\hline
Điều kiện & Người dùng đã đăng nhập và có vai trò ADMIN trong Board. \\
\hline
Luồng chính & 1. Người dùng chọn xóa Board.\newline 2. Hệ thống hiển thị hộp thoại xác nhận xóa.\newline 3. Người dùng xác nhận thao tác.\newline 4. Hệ thống xóa Board và toàn bộ List, Card, Checklist, ChecklistItem bên trong theo cơ chế cascade.\newline 5. Hệ thống chuyển hướng người dùng về trang Workspace. \\
\hline
...
Tác nhân chính & Người dùng \\
\hline
Mô tả tóm tắt & Người dùng có vai trò ADMIN hoặc MEMBER trong Board thêm bất kỳ người dùng đã đăng ký vào Board với vai trò mặc định MEMBER. \\
\hline
Điều kiện & Người dùng đã đăng nhập và có vai trò ADMIN hoặc MEMBER trong Board. \\
\hline
Luồng chính & 1. Người dùng truy cập trang quản lý thành viên Board.\newline 2. Người dùng tìm kiếm người dùng cần thêm theo email (tìm trong toàn bộ hệ thống).\newline 3. Người dùng chọn người dùng từ kết quả tìm kiếm và nhấn Thêm.\newline 4. Hệ thống xác minh người dùng tồn tại và chưa có trong Board.\newline 5. Hệ thống thêm thành viên vào Board với vai trò mặc định MEMBER và cập nhật danh sách. \\
\hline
Luồng thay thế & Thành viên đã có trong Board: Hệ thống hiển thị thông báo trùng lặp (HTTP 409).\newline Người yêu cầu có vai trò VIEWER: Hệ thống trả về lỗi HTTP 403. \\
\hline
\end{tabular}
...
Tác nhân chính & Người dùng \\
\hline
Mô tả tóm tắt & Người dùng xóa List và toàn bộ Card, Checklist, ChecklistItem bên trong theo cơ chế cascade. \\
\hline
Điều kiện & Người dùng đã đăng nhập và có vai trò ADMIN hoặc MEMBER trong Board. \\
\hline
Luồng chính & 1. Người dùng chọn xóa List.\newline 2. Hệ thống hiển thị hộp thoại xác nhận.\newline 3. Người dùng xác nhận thao tác.\newline 4. Hệ thống xóa List và toàn bộ Card, Checklist, ChecklistItem bên trong theo cơ chế cascade, cập nhật giao diện Board. \\
\hline
...
Điều kiện & Người dùng đã đăng nhập và có vai trò ADMIN hoặc MEMBER trong Board. \\
\hline
Luồng chính & 1. Người dùng nhấn giữ và kéo List sang vị trí mong muốn.\newline 2. Giao diện cập nhật thứ tự List ngay lập tức (optimistic update).\newline 3. Hệ thống tính toán và lưu vị trí mới của List vào cơ sở dữ liệu. \\
\hline
...
Điều kiện & Người dùng đã đăng nhập và là thành viên của Board. \\
\hline
Luồng chính & 1. Người dùng nhấp vào Card.\newline 2. Hệ thống tải đầy đủ thông tin Card bao gồm tiêu đề, mô tả, ngày bắt đầu, ngày đến hạn, trạng thái hoàn thành, thành viên phụ trách và danh sách Checklist.\newline 3. Hệ thống hiển thị modal chi tiết Card. \\
\hline
...
Điều kiện & Người dùng đã đăng nhập và có vai trò ADMIN hoặc MEMBER trong Board. \\
\hline
Luồng chính & 1. Người dùng mở modal chi tiết Card.\newline 2. Người dùng tích vào ô đánh dấu hoàn thành.\newline 3. Hệ thống cập nhật trạng thái hoàn thành và phản ánh thay đổi trên giao diện Card. \\
\hline
...
Điều kiện & Người dùng đã đăng nhập và có vai trò ADMIN hoặc MEMBER trong Board. \\
\hline
Luồng chính & 1. Người dùng nhấn giữ và kéo Card sang List mới hoặc vị trí khác trong cùng List.\newline 2. Giao diện cập nhật vị trí Card ngay lập tức (optimistic update).\newline 3. Hệ thống lưu List đích và vị trí mới của Card vào cơ sở dữ liệu. \\
\hline
...
Điều kiện & Người dùng đã đăng nhập và có vai trò ADMIN hoặc MEMBER trong Board. \\
\hline
Luồng chính & 1. Người dùng mở modal chi tiết Card và chọn xóa.\newline 2. Hệ thống hiển thị hộp thoại xác nhận.\newline 3. Người dùng xác nhận thao tác.\newline 4. Hệ thống xóa Card và toàn bộ dữ liệu liên quan, đóng modal và cập nhật giao diện Board. \\
\hline
Luồng thay thế & Card không tồn tại: Hệ thống trả về lỗi HTTP 404.\newline Người dùng có vai trò VIEWER: Hệ thống trả về lỗi HTTP 403.\newline Lỗi không xác định: Hệ thống trả về lỗi HTTP 500. \\
\hline
\end{tabular}
...
\hline
Điều kiện & Người dùng đã đăng nhập và có vai trò ADMIN hoặc MEMBER trong Board; Card đang mở. \\
\hline
Luồng chính & 1. Người dùng chọn thêm Checklist trong modal Card.\newline 2. Hệ thống hiển thị ô nhập tên Checklist.\newline 3. Người dùng nhập tên và nhấn Thêm.\newline 4. Hệ thống tạo Checklist và hiển thị ngay trong modal Card. \\
\hline
...
\hline
Điều kiện & Người dùng đã đăng nhập và có vai trò ADMIN hoặc MEMBER trong Board; Checklist tồn tại. \\
\hline
Luồng chính & 1. Người dùng nhấp vào tên Checklist để chỉnh sửa.\newline 2. Người dùng nhập tên mới và xác nhận.\newline 3. Hệ thống cập nhật tên Checklist và hiển thị kết quả. \\
\hline
...
Điều kiện & Người dùng đã đăng nhập và có vai trò ADMIN hoặc MEMBER trong Board; Checklist tồn tại. \\
\hline
Luồng chính & 1. Người dùng chọn xóa Checklist.\newline 2. Hệ thống hiển thị hộp thoại xác nhận.\newline 3. Người dùng xác nhận thao tác.\newline 4. Hệ thống xóa Checklist và toàn bộ ChecklistItem bên trong, cập nhật modal Card. \\
\hline
...
\hline
Điều kiện & Người dùng đã đăng nhập và có vai trò ADMIN hoặc MEMBER trong Board; ChecklistItem tồn tại. \\
\hline
Luồng chính & 1. Người dùng nhấp vào tên hạng mục để chỉnh sửa.\newline 2. Người dùng thay đổi tên hoặc ngày đến hạn.\newline 3. Người dùng xác nhận thay đổi.\newline 4. Hệ thống xác thực và lưu thông tin mới, hiển thị kết quả cập nhật. \\
\hline
...
Điều kiện & Người dùng đã đăng nhập và có vai trò ADMIN hoặc MEMBER trong Board; ChecklistItem tồn tại. \\
\hline
Luồng chính & 1. Người dùng tích vào ô bên cạnh tên hạng mục.\newline 2. Hệ thống cập nhật trạng thái hoàn thành của hạng mục.\newline 3. Hệ thống tính lại tỷ lệ hoàn thành và cập nhật thanh tiến độ. \\
\hline
...
Điều kiện & Người dùng đã đăng nhập và có vai trò ADMIN hoặc MEMBER trong Board; ChecklistItem tồn tại. \\
\hline
Luồng chính & 1. Người dùng chọn xóa hạng mục.\newline 2. Hệ thống xóa hạng mục khỏi Checklist.\newline 3. Hệ thống tính lại tỷ lệ hoàn thành và cập nhật thanh tiến độ. \\
\hline
...
\hline
Điều kiện & Người dùng đã đăng nhập và có vai trò ADMIN hoặc MEMBER trong Board; thành viên đã được gán vào Card. \\
\hline
Luồng chính & 1. Người dùng mở modal Card và truy cập danh sách thành viên đã gán.\newline 2. Người dùng chọn thành viên cần hủy gán.\newline 3. Hệ thống hủy gán thành viên và cập nhật danh sách, xóa avatar khỏi Card. \\
\hline
...
Phần này trình bày biểu đồ hoạt động cho các use case quan trọng và có luồng xử lý phức tạp trong hệ thống Nello. Các use case có quy trình đơn giản hoặc tương tự với use case đã được trình bày không được lặp lại biểu đồ riêng.

% ── UC-01 ──────────────────────────────────────────────────────────────────

\paragraph{UC-01: Quản lý xác thực} \mbox{}\\

Nhóm chức năng xác thực trình bày biểu đồ hoạt động cho ba luồng: đăng ký tài khoản (UC-01.1), xác nhận OTP (UC-01.2) và đăng nhập (UC-01.4).
...
\medskip\noindent\textbf{UC-01.2: Xác nhận OTP}\medskip

Người dùng nhập mã OTP đã nhận qua email; hệ thống kiểm tra tính hợp lệ và thời hạn của mã OTP, sau đó ủy quyền cho Amazon Cognito kích hoạt tài khoản trước khi chuyển hướng đến trang đăng nhập.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth, height=0.85\textheight, keepaspectratio]{images/chap3/activity_uc01_2.png}
\caption{Biểu đồ hoạt động UC-01.2: Xác nhận OTP}
...
Nhóm chức năng quản lý Workspace trình bày biểu đồ hoạt động cho hai luồng: tạo Workspace (UC-03.1) và mời thành viên vào Workspace (UC-03.4).
...
Người dùng có quyền (ADMIN hoặc MEMBER trong Workspace) nhập email người dùng cần mời; hệ thống xác minh email tồn tại trong hệ thống, thêm thành viên với vai trò MEMBER và thông báo kết quả.
...
Nhóm chức năng quản lý Board trình bày biểu đồ hoạt động cho ba luồng: tạo Board (UC-04.1), xóa Board (UC-04.4) và thêm thành viên vào Board (UC-04.5).
...
Sau khi xác nhận xóa, hệ thống xóa Board cùng toàn bộ dữ liệu liên quan (List, Card, Checklist, ChecklistItem) theo cơ chế cascade và chuyển hướng về trang Workspace.
...
Người dùng có quyền (ADMIN hoặc MEMBER trong Board) tìm kiếm bất kỳ người dùng nào trong hệ thống theo email và thêm vào Board; vai trò được gán mặc định là MEMBER.
...
Nhóm chức năng quản lý List trình bày biểu đồ hoạt động cho ba luồng: tạo List (UC-05.1), xóa List (UC-05.3) và sắp xếp thứ tự List (UC-05.4).
...
Nhóm chức năng quản lý Card trình bày biểu đồ hoạt động cho ba luồng: tạo Card (UC-06.1), di chuyển Card (UC-06.5) và xóa Card (UC-06.6).
...
Nhóm chức năng quản lý Checklist trình bày biểu đồ hoạt động cho ba luồng: tạo Checklist (UC-07.1), tạo Checklist Item (UC-07.4) và đánh dấu Item hoàn thành (UC-07.6).
...
Nhóm chức năng quản lý thành viên Card trình bày biểu đồ hoạt động cho luồng gán thành viên vào Card (UC-08.1).
...
Hệ thống xoay quanh bảy thực thể nghiệp vụ chính được tổ chức theo cấu trúc phân cấp:
...
\begin{itemize}
    ...
    \item \textbf{Workspace (Không gian làm việc):} Đơn vị tổ chức cấp cao nhất, quản lý tập trung nhiều Board liên quan. Mỗi Workspace có một người tạo (được gán vai trò ADMIN) và nhiều thành viên với các vai trò ADMIN, MEMBER hoặc VIEWER.
...
\paragraph{Hạ tầng triển khai} \mbox{}\\
...
Workspace là đơn vị tổ chức cấp cao nhất trong hệ thống. Sau khi Backend xác thực JWT, hệ thống thực hiện hai thao tác ghi tuần tự: tạo bản ghi Workspace và tự động ghi nhận người tạo là thành viên với vai trò \texttt{ADMIN} trong bảng \texttt{workspace\_members}.
...
Board được tạo trong phạm vi một Workspace. Backend kiểm tra người dùng có phải thành viên Workspace với vai trò ADMIN hoặc MEMBER không — người không thuộc Workspace hoặc có vai trò VIEWER đều bị từ chối với \texttt{403 Forbidden}. Sau khi Board được tạo, người tạo được ghi nhận tự động là thành viên Board với vai trò \texttt{ADMIN}.
...
List là các cột trong giao diện Kanban. Backend kiểm tra vai trò của người dùng trong Board trước khi thực hiện INSERT — người dùng có vai trò VIEWER bị từ chối với \texttt{403 Forbidden}; chỉ ADMIN hoặc MEMBER mới được tạo List. Trường \texttt{position} sử dụng giá trị chuỗi phân số (fractional indexing) để hỗ trợ sắp xếp linh hoạt mà không cần cập nhật lại toàn bộ danh sách.
...
Card là đơn vị công việc cơ bản trong hệ thống. Backend thực hiện hai bước kiểm tra tuần tự trước khi ghi: xác minh List đích tồn tại và xác minh người dùng có vai trò ADMIN hoặc MEMBER trong Board (VIEWER bị từ chối với \texttt{403 Forbidden}). Cơ chế phân quyền này đảm bảo mọi thao tác tạo dữ liệu đều được kiểm soát chặt chẽ.
...
Luồng gồm hai giai đoạn: tìm kiếm user theo email và thêm vào Board. Backend xác minh người yêu cầu có vai trò ADMIN hoặc MEMBER trong Board (VIEWER bị từ chối với \texttt{403 Forbidden}), kiểm tra user đích chưa là thành viên Board (nếu đã có trả về \texttt{409 Conflict}). Bất kỳ người dùng đã đăng ký đều có thể được thêm vào Board — không yêu cầu là thành viên Workspace.
...
Luồng này minh họa kỹ thuật \textbf{optimistic update} — giao diện được cập nhật tức thì ngay khi người dùng kéo thả Card, trước khi nhận phản hồi từ Backend, nhằm tối ưu trải nghiệm người dùng. Backend kiểm tra vai trò người dùng trong Board trước khi thực hiện UPDATE — VIEWER bị từ chối với \texttt{403 Forbidden}. Nếu yêu cầu thất bại vì bất kỳ lý do nào, Frontend tự động rollback giao diện về trạng thái trước đó.
...
Tương tự SD-09, luồng di chuyển List cũng áp dụng kỹ thuật \textbf{optimistic update}. Khi người dùng kéo thả một List sang vị trí khác trong Board, Frontend cập nhật giao diện ngay lập tức trong khi Backend kiểm tra vai trò người dùng (VIEWER bị từ chối \texttt{403 Forbidden}) rồi cập nhật trường \texttt{position} vào cơ sở dữ liệu. Nếu thao tác thất bại, UI được rollback về trạng thái ban đầu.
...
Cơ sở dữ liệu của hệ thống Nello sử dụng PostgreSQL, bao gồm 13 bảng được tổ chức thành ba nhóm: bảng tra cứu dùng chung, chuỗi phân cấp nghiệp vụ và bảng trung gian quản lý thành viên. Khóa chính của các bảng nghiệp vụ sử dụng kiểu UUID để đảm bảo tính duy nhất toàn cục; các bảng tra cứu sử dụng INT. Cơ chế xóa theo tầng (cascade delete) được áp dụng nhất quán theo chuỗi: Workspace $\rightarrow$ Board $\rightarrow$ List $\rightarrow$ Card $\rightarrow$ Checklist $\rightarrow$ ChecklistItem.
...
\begin{figure}[p]
    \centering
    \vspace*{-5cm}
    \noindent\makebox[\textwidth][c]{\includegraphics[width=\paperwidth, height=\paperheight, keepaspectratio]{./images/DB_Design.png}}
...
\subparagraph{Bảng \texttt{roles}} \mbox{}

\hspace{1.5em}\textbf{a. Cấu trúc bảng}
...
\subparagraph{Bảng \texttt{images}} \mbox{}
...
\subparagraph{Bảng \texttt{workspace\_categories}} \mbox{}
...
\subparagraph{Bảng \texttt{users}} \mbox{}

\hspace{1.5em}\textbf{a. Mô tả}
...
\hspace{1.5em}\textbf{b. Cấu trúc bảng}
...
\subparagraph{Bảng \texttt{workspaces}} \mbox{}
...
\subparagraph{Bảng \texttt{workspace\_members}} \mbox{}
...
\subparagraph{Bảng \texttt{boards}} \mbox{}
...
\subparagraph{Bảng \texttt{board\_members}} \mbox{}
...
Ràng buộc UNIQUE trên cặp (\texttt{user\_id}, \texttt{board\_id}) đảm bảo mỗi người dùng chỉ có một vai trò duy nhất trong một Board. Phân quyền Board hoạt động độc lập với phân quyền Workspace — bất kỳ người dùng nào đã đăng ký tài khoản đều có thể được thêm vào Board mà không cần là thành viên của Workspace chứa Board đó.


\subparagraph{Bảng \texttt{lists}} \mbox{}
...
\subparagraph{Bảng \texttt{cards}} \mbox{}
...
\subparagraph{Bảng \texttt{card\_members}} \mbox{}
...
\subparagraph{Bảng \texttt{checklists}} \mbox{}
...
\subparagraph{Bảng \texttt{checklist\_items}} \mbox{}
...
\hspace{1.5em}\textbf{c. Ghi chú}

Kiểu dữ liệu BOOLEAN của trường \texttt{is\_completed} đã đảm bảo chỉ nhận hai giá trị hợp lệ: \texttt{true} hoặc \texttt{false}. Giá trị mặc định là \texttt{false} — mỗi hạng mục khi được tạo ra ở trạng thái chưa hoàn thành.
...
Sau 8 tuần thực hiện (từ 30/03 đến 24/05/2026), hệ thống Nello đã hoàn thành \textbf{27/31 chức năng} yêu cầu, đạt tỷ lệ \textbf{87,1\%}. Bốn chức năng còn lại (FR-08, FR-12, FR-30, FR-31) đã được triển khai hoàn chỉnh ở tầng Backend nhưng giao diện Frontend tương ứng chưa được xây dựng trong thời gian thực hiện đề tài.
...
Quá trình kiểm thử được thực hiện theo từng tuần phát triển với tổng cộng hơn 30 test case. Phần lớn tập trung vào luồng thành công (happy case) và một số trường hợp lỗi cơ bản.

\begin{table}[H]
\centering
\begin{tabular}{|p{3.5cm}|p{1.5cm}|p{7cm}|p{1.5cm}|}
\hline
\textbf{Module} & \textbf{Số TC} & \textbf{Kịch bản tiêu biểu} & \textbf{Kết quả} \\
\hline
Authentication (W3) & 14 & Đăng ký, xác nhận OTP, đăng nhập, làm mới token, gửi lại mã & Pass \\
\hline
Workspace \& Board (W4) & 16 & CRUD Workspace, CRUD Board (bao gồm kiểm tra quyền), CRUD List & Pass \\
\hline
Card \& Checklist (W5) & 9 & CRUD Card, drag-drop Card, CRUD Checklist và ChecklistItem & Pass \\
\hline
Frontend tích hợp (W6) & 9 & Luồng xác thực end-to-end, Dashboard, tạo Workspace và Board & Pass \\
\hline
Giao diện Board (W7) & 7 & Workspace Detail, Board Kanban: tạo/xóa List và Card & Pass \\
\hline
\end{tabular}
\caption{Tổng hợp kết quả kiểm thử theo module}
...
Số liệu triển khai tổng hợp đã được trình bày tại Bảng~\ref{tab:results_summary} (Chương 4). Đối chiếu với các mục tiêu đề ra ban đầu, các kết quả cụ thể đạt được như sau:
...
Các hạn chế còn tồn tại (được liệt kê chi tiết tại mục 4.4.2) có thể được phân nhóm theo nguyên nhân gốc như sau:

\textbf{Nhóm 1 — Hạn chế do ràng buộc thời gian:}
Một số tính năng đã được thiết kế và triển khai hoàn chỉnh ở tầng Backend nhưng giao diện Frontend tương ứng chưa kịp hoàn thiện trong thời gian 8 tuần. Nguyên nhân là giao diện Kanban Board phức tạp hơn dự kiến, dẫn đến việc ưu tiên tập trung vào các chức năng cốt lõi (Workspace, Board, List, Card, Drag-and-Drop, Checklist) thay vì phủ đều toàn bộ FR.

\textbf{Nhóm 2 — Hạn chế do quyết định thiết kế sớm:}
Vấn đề triển khai lên môi trường Staging xuất phát từ quyết định dùng HttpOnly cookie để lưu token — một lựa chọn đúng về bảo mật nhưng gặp xung đột SameSite khi Frontend và Backend ở các domain khác nhau. Đây là hạn chế cần giải quyết ở giai đoạn tiếp theo bằng cách cấu hình \texttt{SameSite=None; Secure} hoặc sử dụng cùng domain cho cả hai thành phần.

\textbf{Nhóm 3 — Hạn chế do thiếu hạ tầng bổ sung:}
Việc chưa có đồng bộ thời gian thực và hệ thống thông báo không phải là thiếu sót trong thiết kế ban đầu mà là tính năng nằm ngoài phạm vi đề tài. Để triển khai, cần bổ sung WebSocket (Spring WebSocket + STOMP) vào Backend và tích hợp SockJS/STOMP client vào Frontend — đây là khối lượng công việc đáng kể, phù hợp cho các giai đoạn tiếp theo.

\textbf{Nhóm 4 — Hạn chế do chiến lược kiểm thử:}
Phần lớn test case tập trung vào happy case và lỗi cơ bản. Điều này phản ánh thực tế của quá trình phát triển theo tuần với tài nguyên hạn chế — các trường hợp biên và điều kiện lỗi nâng cao cần được bổ sung trong chu kỳ kiểm thử tiếp theo.
